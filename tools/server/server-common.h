...
#define SRV_INF(fmt, ...) LOG_INF("srv  %12.*s: " fmt, 12, __func__ __VA_OPT__(,) __VA_ARGS__)
#define SRV_WRN(fmt, ...) LOG_WRN("srv  %12.*s: " fmt, 12, __func__ __VA_OPT__(,) __VA_ARGS__)
#define SRV_ERR(fmt, ...) LOG_ERR("srv  %12.*s: " fmt, 12, __func__ __VA_OPT__(,) __VA_ARGS__)
#define SRV_DBG(fmt, ...) LOG_DBG("srv  %12.*s: " fmt, 12, __func__ __VA_OPT__(,) __VA_ARGS__)
...
struct server_tokens {
    ...
    // For pi0 models: owns the original full mtmd_input_chunks so that
    // mtmd_helper_eval_chunks_pi0() can receive the complete text+image list.
    // Empty for non-pi0 paths or when not tokenized via process_mtmd_prompt().
    mtmd::input_chunks_ptr owned_raw_chunks;

    // For pi0 requests: ensure the full chunk group is only evaluated once per request.
    mutable bool pi0_chunks_processed = false;
...
public:
    ...
    // when is_pi0 is true, uses mtmd_helper_eval_chunks_pi0 instead of mtmd_helper_eval_chunk_single
    int32_t process_chunk(
                ...
                size_t & n_tokens_out,
                bool is_pi0 = false,
                mtmd_pi0_result * pi0_result = nullptr) const;
};

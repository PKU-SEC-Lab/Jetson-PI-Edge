...
task_params server_task::params_from_json_cmpl(
    ...
    const bool model_is_pi0 = !params_base.mmproj.path.empty() && llama_model_has_encoder(model) && llama_model_has_decoder(model);
    ...
    params.is_pi0 = model_is_pi0;

    // pi0 model state injection
    // accepts a JSON array of floats OR a comma-separated string, e.g.:
    //   "state": [0.1, 0.2, 0.3, ...]
    //   "state": "0.1,0.2,0.3,..."
    if (data.contains("state")) {
        const auto & state_val = data.at("state");
        if (state_val.is_array()) {
            for (const auto & v : state_val) {
                if (v.is_number()) {
                    params.pi0_state.push_back(v.get<float>());
                }
            }
        } else if (state_val.is_string()) {
            const std::string & s = state_val.get<std::string>();
            ...
            while (std::getline(iss, tok, ',')) {
                ...
                if (!tok.empty()) {
                    params.pi0_state.push_back(std::stof(tok));
                }
            }
        }
    }
...
json server_task_result_cmpl_final::pi0_result_to_json() const {
    json res = {
        {"type", "pi0_action"},
        {"source", "llama_context.cross.action"},
    };

    if (pi0_result.has_action) {
        json action = json::array();
        const int32_t action_dim = pi0_result.action_dim;
        const int32_t action_steps = pi0_result.action_steps;

        for (int32_t step = 0; step < action_steps; ++step) {
            json step_values = json::array();
            const int64_t offset = (int64_t) step * action_dim;
            for (int32_t dim = 0; dim < action_dim; ++dim) {
                step_values.push_back(pi0_result.action[offset + dim]);
            }
            action.push_back(step_values);
        }

        res["action_dim"] = action_dim;
        res["action_steps"] = action_steps;
        res["action"] = std::move(action);
    }

    json timing = json::object();
    if (pi0_result.has_vit_ms) {
        timing["vit_ms"] = pi0_result.vit_ms;
    }
    if (pi0_result.has_encode_ms) {
        timing["encode_ms"] = pi0_result.encode_ms;
    }
    if (pi0_result.has_decode_ms) {
        timing["decode_ms"] = pi0_result.decode_ms;
    }
    if (pi0_result.has_total_ms) {
        timing["total_ms"] = pi0_result.total_ms;
    }
    if (!timing.empty()) {
        res["timing"] = std::move(timing);
    }
...
json server_task_result_cmpl_final::to_json_non_oaicompat() {
    ...
    if (is_pi0) {
        res["pi0_result"] = pi0_result_to_json();
    }

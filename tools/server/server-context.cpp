...
static pi_model_kind server_pi_model_kind() {
    return pi_model_kind_from_env();
}

static bool server_pi_model_use_pi05_adapters() {
    return pi_model_use_pi05_adapters_by_default(server_pi_model_kind()) != 0;
}

static bool server_request_has_pi05_payload(const json & body) {
    return body.contains("head_image_b64") ||
           body.contains("left_image_b64") ||
           body.contains("right_image_b64");
}

static void server_pi_model_debug_dump_line(const std::string & line) {
...
static bool server_set_pi_model_env(pi_model_kind kind) {
    if (kind != PI_MODEL_PI0 && kind != PI_MODEL_PI05) {
        return false;
    }
    const char * name = pi_model_kind_name(kind);
#if defined(_WIN32)
    return _putenv_s("PI_MODEL", name) == 0;
#else
    return setenv("PI_MODEL", name, 1) == 0;
#endif
}

static bool server_resolve_pi_model_before_load(const common_params & params) {
...
struct pi0_action_norm_stats {
    std::vector<float> q01;
    std::vector<float> q99;
    bool loaded = false;
};

static std::string build_pi05_openpi_prompt(
        const std::string & raw_text,
        const std::vector<float> & state) {
    std::ostringstream oss;

    oss << "Task: " << raw_text << ", State: ";

    const int n_state_for_text = std::min<int>(8, state.size());
    for (int i = 0; i < n_state_for_text; ++i) {
        ...
        if (x < -1.0f) {
            ...
            bin = int(std::floor((x + 1.0f) * 128.0f));
            if (bin < 0) bin = 0;
            if (bin > 255) bin = 255;
        }
...
static pi0_action_norm_stats load_pi0_action_norm_stats_once() {
    pi0_action_norm_stats stats;
    const char * path = std::getenv("PI0_NORM_STATS_JSON");
    ...
    if (!in) {
        LOG_WRN("failed to open PI0_NORM_STATS_JSON=%s; returning raw pi0 actions\n", path);
        return stats;
    }
    ...
    try {
        ...
        if (node->contains("norm_stats")) {
            node = &node->at("norm_stats");
        }
        if (!node->contains("actions")) {
            LOG_WRN("PI0_NORM_STATS_JSON=%s has no actions stats; returning raw pi0 actions\n", path);
            return stats;
        }
        const json & actions = node->at("actions");
        if (!actions.contains("q01") || !actions.contains("q99") || !actions.at("q01").is_array() || !actions.at("q99").is_array()) {
            LOG_WRN("PI0_NORM_STATS_JSON=%s has no actions q01/q99 arrays; returning raw pi0 actions\n", path);
            return stats;
        }

        for (const auto & v : actions.at("q01")) {
            stats.q01.push_back(v.get<float>());
        }
        for (const auto & v : actions.at("q99")) {
            stats.q99.push_back(v.get<float>());
        }

        if (stats.q01.empty() || stats.q01.size() != stats.q99.size()) {
            stats.q01.clear();
            stats.q99.clear();
            LOG_WRN("PI0_NORM_STATS_JSON=%s has invalid q01/q99 lengths; returning raw pi0 actions\n", path);
            return stats;
        }

        stats.loaded = true;
        LOG_INF("loaded pi0 action quantile norm stats from %s with dim=%zu\n", path, stats.q01.size());
    } catch (const std::exception & e) {
        stats.q01.clear();
        stats.q99.clear();
        stats.loaded = false;
        LOG_WRN("failed to parse PI0_NORM_STATS_JSON=%s: %s; returning raw pi0 actions\n", path, e.what());
    }

    return stats;
}

static const pi0_action_norm_stats & get_pi0_action_norm_stats() {
    static const pi0_action_norm_stats stats = load_pi0_action_norm_stats_once();
    return stats;
}

static bool server_pi_model_skip_pi0_action_unnormalize() {
    return pi_model_kind_is_pi0(server_pi_model_kind()) != 0;
}

static float maybe_unnormalize_pi0_action(float value, size_t dim) {
    if (server_pi_model_skip_pi0_action_unnormalize()) {
        return value;
    }

    const auto & stats = get_pi0_action_norm_stats();
    if (!stats.loaded || dim >= stats.q01.size()) {
        return value;
    }

    return (value + 1.0f) * 0.5f * (stats.q99[dim] - stats.q01[dim] + 1e-6f) + stats.q01[dim];
}

static json pi0_action_norm_debug_json() {
    const auto & stats = get_pi0_action_norm_stats();
    json out = json::object();
    out["loaded"] = stats.loaded;
    out["dim"] = stats.q01.size();
    out["pi_model"] = pi_model_kind_name(server_pi_model_kind());
    out["unnormalize_skipped"] = server_pi_model_skip_pi0_action_unnormalize();
    out["selected"] = json::array();

    static constexpr size_t selected_dims[] = {9, 19, 12, 11, 7, 8, 6, 10};
    for (const size_t dim : selected_dims) {
        if (!stats.loaded || dim >= stats.q01.size()) {
            continue;
        }

        const float q01 = stats.q01[dim];
        const float q99 = stats.q99[dim];
        out["selected"].push_back({
            {"dim", dim},
            {"q01", q01},
            {"q99", q99},
            {"scale", 0.5f * (q99 - q01 + 1e-6f)},
            {"center", 0.5f * (q99 + q01)},
        });
    }
...
struct server_context_impl {
    ...
    struct pi0_http_infer_request {
        std::vector<float> state;
        std::string prompt;
        std::string image_encoding;
        uint32_t image_width  = 0;
        uint32_t image_height = 0;
        std::string head_image_b64;
        std::string left_image_b64;
        std::string right_image_b64;
    };
    ...
    struct foreground_session {
        ...
        ~foreground_session() {
            mtmd_pi0_result_free(&latest_pi0_result);
    ...
    static std::string strip_data_url_prefix(const std::string & value) {
        ...
        const std::string prefix = value.substr(0, comma_pos);
    ...
    static std::string ensure_data_url_base64(const std::string & value) {
        if (value.empty()) {
            throw std::runtime_error("base64 image payload is empty");
        }

        if (value.rfind("data:", 0) == 0) {
            if (value.find(";base64,") == std::string::npos) {
                throw std::runtime_error("base64 image payload must use data URL base64 encoding");
            }
            return value;
        }

        return "data:image/png;base64," + strip_data_url_prefix(value);
    }

    static inline bool is_base64_char(uint8_t c) {
        return (isalnum(c) || (c == '+') || (c == '/'));
    }

    static raw_buffer decode_base64_bytes(const std::string & encoded_string) {
    ...
    static pi0_http_infer_request parse_pi0_http_infer_request(const json & body) {
        pi0_http_infer_request req;

        if (!body.contains("state") || !body.at("state").is_array()) {
            throw std::runtime_error("\"state\" must be a float array");
        }

        for (const auto & v : body.at("state")) {
            if (!v.is_number()) {
                throw std::runtime_error("\"state\" must contain only numeric values");
            }
            req.state.push_back(v.get<float>());
        }

        req.prompt = json_value(body, "prompt", std::string());
        if (req.prompt.empty()) {
            req.prompt = json_value(body, "text", std::string());
        }
        if (req.prompt.empty()) {
            throw std::runtime_error("\"prompt\" or \"text\" is required for pi0 HTTP inference");
        }

        req.image_encoding = json_value(body, "image_encoding", std::string());
        req.image_width    = json_value(body, "image_width",  (uint32_t) 0);
        req.image_height   = json_value(body, "image_height", (uint32_t) 0);

        req.head_image_b64 = ensure_data_url_base64(json_value(body, "head_image_b64", std::string()));
        req.left_image_b64 = ensure_data_url_base64(json_value(body, "left_image_b64", std::string()));
        req.right_image_b64 = ensure_data_url_base64(json_value(body, "right_image_b64", std::string()));

        return req;
    }

    static mtmd::bitmap decode_pi0_image_b64(
            mtmd_context * mctx,
            const pi0_http_infer_request & req,
            const std::string & image_b64) {
        const raw_buffer decoded = decode_base64_bytes(strip_data_url_prefix(image_b64));

        if (req.image_encoding == "rgb_u8") {
            if (req.image_width == 0 || req.image_height == 0) {
                throw std::runtime_error("\"image_width\" and \"image_height\" are required when image_encoding is rgb_u8");
            }

            const size_t expected = (size_t) req.image_width * (size_t) req.image_height * 3;
            if (decoded.size() != expected) {
                throw std::runtime_error(
                        "rgb_u8 image byte size mismatch: expected " + std::to_string(expected) +
                        ", got " + std::to_string(decoded.size()));
            }

            mtmd_bitmap * raw = mtmd_bitmap_init(req.image_width, req.image_height, decoded.data());
            if (raw == nullptr) {
                throw std::runtime_error("failed to allocate pi0 rgb_u8 bitmap");
            }
            return mtmd::bitmap(raw);
        }

        mtmd_bitmap * raw = mtmd_helper_bitmap_init_from_buf(mctx, decoded.data(), decoded.size());
        if (raw == nullptr) {
            throw std::runtime_error("failed to decode image (expected png/jpeg or rgb_u8 with image_encoding)");
        }
        return mtmd::bitmap(raw);
    }

static json reshape_action_rows(const mtmd_pi0_result & pi0_result, bool final_action) {
json action_rows = json::array();
const bool has_action = final_action ? pi0_result.has_action_final : pi0_result.has_action;
const float * action_ptr = final_action ? pi0_result.action_final_data : pi0_result.action_data;

        if (!has_action || action_ptr == nullptr || pi0_result.action_dim <= 0 || pi0_result.action_steps <= 0) {
            return action_rows;
        }

        for (int32_t step = 0; step < pi0_result.action_steps; ++step) {
            json step_values = json::array();
            const int64_t offset = (int64_t) step * pi0_result.action_dim;
            for (int32_t dim = 0; dim < pi0_result.action_dim; ++dim) {
                const float value = action_ptr[offset + dim];
                step_values.push_back(final_action ? maybe_unnormalize_pi0_action(value, (size_t) dim) : value);
            }
            action_rows.push_back(std::move(step_values));
        }

return action_rows;
}

static json reshape_action_raw_rows(const mtmd_pi0_result & pi0_result, bool final_action) {
    json action_rows = json::array();
    const bool has_action = final_action ? pi0_result.has_action_final : pi0_result.has_action;
    const float * action_ptr = final_action ? pi0_result.action_final_data : pi0_result.action_data;
    ...
    for (int32_t step = 0; step < pi0_result.action_steps; ++step) {
        ...
        const int64_t offset = (int64_t) step * pi0_result.action_dim;
        for (int32_t dim = 0; dim < pi0_result.action_dim; ++dim) {
            step_values.push_back(action_ptr[offset + dim]);
        }
        action_rows.push_back(std::move(step_values));
    }
...
static json slice_joint_command_rows(const mtmd_pi0_result & pi0_result, bool final_action) {
json action_rows = reshape_action_rows(pi0_result, final_action);
json sliced_rows = json::array();

        for (const auto & row_value : action_rows) {
            if (!row_value.is_array()) {
                continue;
            }

            json sliced = json::array();
            const auto & row = row_value;
            const size_t row_size = row.size();

            const size_t joint_keep = std::min<size_t>(32, row_size);
            for (size_t i = 0; i < joint_keep; ++i) {
                sliced.push_back(row.at(i));
            }
    ...
    bool foreground_check_antiprompt(const llama_tokens & generated_tokens) const {
        if (foreground.antiprompt_tokens.empty() || generated_tokens.size() < foreground.antiprompt_tokens.size()) {
            return false;
        }

        return std::equal(
                generated_tokens.end() - foreground.antiprompt_tokens.size(),
                generated_tokens.end(),
                foreground.antiprompt_tokens.begin());
    }

    bool foreground_load_media(const std::string & fname) {
        mtmd::bitmap bmp(mtmd_helper_bitmap_init_from_file(mctx, fname.c_str()));
        if (!bmp.ptr) {
            return false;
        }

        foreground.bitmaps.entries.push_back(std::move(bmp));
        return true;
    }

    bool foreground_use_jinja() const {
        return params_base.use_jinja
            && params_base.chat_template != "vicuna"
            && params_base.chat_template != "deepseek";
    }

    std::string foreground_chat_add_and_format(common_chat_msg & new_msg) {
        auto formatted = common_chat_format_single(
                chat_templates.get(),
                foreground.chat_history,
                new_msg,
                new_msg.role == "user",
                foreground_use_jinja());
        foreground.chat_history.push_back(new_msg);
        return formatted;
    }

    int foreground_eval_message(common_chat_msg & msg, bool is_pi0, json * timing = nullptr) {
        const int64_t t_format_start = ggml_time_us();
        ...
        if (use_pi05_pi0_adapter) {
            ...
            formatted_chat = foreground_chat_add_and_format(msg);
        }
        const int64_t t_format_done = ggml_time_us();
        ...
        auto bitmaps_c_ptr = foreground.bitmaps.c_ptr();
        ...
        if (res != 0) {
            SRV_ERR("Unable to tokenize prompt, res = %d\n", res);
            return 1;
        }

        const int64_t t_clear_bitmaps_start = ggml_time_us();
        foreground.bitmaps.entries.clear();
        const int64_t t_clear_bitmaps_done = ggml_time_us();

        llama_pos new_n_past = 0;
        mtmd_pi0_result * pi0_result = is_pi0 ? &foreground.latest_pi0_result : nullptr;
        if (!is_pi0) {
            mtmd_pi0_result_free(&foreground.latest_pi0_result);
            foreground.latest_state.clear();
        }
        const int64_t t_model_eval_start = ggml_time_us();
        const int32_t eval_res = is_pi0
                ? mtmd_helper_eval_chunks_pi0(mctx, ctx, chunks.ptr.get(), foreground.n_past, 0, params_base.n_batch, true, &new_n_past, pi0_result)
                : mtmd_helper_eval_chunks(mctx, ctx, chunks.ptr.get(), foreground.n_past, 0, params_base.n_batch, true, &new_n_past);
        const int64_t t_model_eval_done = ggml_time_us();
        if (eval_res != 0) {
            SRV_ERR("Unable to eval prompt\n");
            return 1;
        }

        foreground.n_past = new_n_past;
        const int64_t t_state_copy_start = ggml_time_us();
        if (is_pi0 && !use_pi05_pi0_adapter) {
            foreground.latest_state.clear();
            if (foreground.latest_pi0_result.has_state && foreground.latest_pi0_result.state_data != nullptr && foreground.latest_pi0_result.state_dim > 0) {
                foreground.latest_state.assign(
                        foreground.latest_pi0_result.state_data,
                        foreground.latest_pi0_result.state_data + foreground.latest_pi0_result.state_dim);
            }
        } else if (!is_pi0) {
            foreground.latest_state.clear();
        }
        const int64_t t_state_copy_done = ggml_time_us();

        if (timing != nullptr) {
            *timing = {
                {"chat_format_ms",      (t_format_done - t_format_start) / 1000.0},
                {"tokenize_ms",         (t_tokenize_done - t_tokenize_start) / 1000.0},
                {"clear_bitmaps_ms",    (t_clear_bitmaps_done - t_clear_bitmaps_start) / 1000.0},
                {"model_eval_ms",       (t_model_eval_done - t_model_eval_start) / 1000.0},
                {"state_copy_ms",       (t_state_copy_done - t_state_copy_start) / 1000.0},
                {"eval_total_ms",       (t_state_copy_done - t_format_start) / 1000.0},
            };
        }
        return 0;
    }
    ...
    int foreground_generate_response(int n_predict, std::string & generated_text) {
        ...
        for (int i = 0; i < n_predict; ++i) {
            llama_token token_id = common_sampler_sample(foreground.smpl, ctx, -1);
            generated_tokens.push_back(token_id);
            common_sampler_accept(foreground.smpl, token_id, true);

            if (llama_vocab_is_eog(vocab, token_id) || foreground_check_antiprompt(generated_tokens)) {
                break;
            }

            common_batch_clear(foreground.batch);
            common_batch_add(foreground.batch, token_id, foreground.n_past++, {0}, true);
            if (llama_decode(ctx, foreground.batch)) {
                SRV_ERR("failed to decode token\n");
                return 1;
            }
        }

        generated_text = common_detokenize(ctx, generated_tokens);
        common_chat_msg msg;
        msg.role = "assistant";
        msg.content = generated_text;
        foreground.chat_history.push_back(std::move(msg));
        return 0;
    }
    ...
    bool load_model(const common_params & params) {
        ...
        params_base = params;
        if (!server_resolve_pi_model_before_load(params_base)) {
            return false;
        }
        ...
        foreground.smpl = common_sampler_init(model, params_base.sampling);
        ...
        if (params_base.chat_template == "vicuna") {
            foreground.antiprompt_tokens = common_tokenize(ctx, "ASSISTANT:", false, true);
        } else if (params_base.chat_template == "deepseek") {
            foreground.antiprompt_tokens = common_tokenize(ctx, "###", false, true);
        }
    ...
    bool launch_slot_with_task(server_slot & slot, server_task && task) {
        ...
        // pi0 model: inject proprioception / robot state into the llama context
        // before moving task into slot (accessing task.params after move is UB)
        if (task.params.is_pi0) {
            SLT_INF(slot, "pi0 mode: injecting state vector of %zu floats\n", task.params.pi0_state.size());
            llama_set_pi0_state(slot.ctx, task.params.pi0_state.data(), task.params.pi0_state.size());
        }
    ...
    void send_final_response(server_slot & slot) {
        ...
        res->is_pi0 = slot.task->params.is_pi0;
        if (res->is_pi0) {
            res->pi0_result.vit_ms = slot.pi0_result.vit_ms;
            res->pi0_result.encode_ms = slot.pi0_result.encode_ms;
            res->pi0_result.decode_ms = slot.pi0_result.decode_ms;
            res->pi0_result.total_ms = slot.pi0_result.total_ms;
            res->pi0_result.has_vit_ms = slot.pi0_result.has_vit_ms;
            res->pi0_result.has_encode_ms = slot.pi0_result.has_encode_ms;
            res->pi0_result.has_decode_ms = slot.pi0_result.has_decode_ms;
            res->pi0_result.has_total_ms = slot.pi0_result.has_total_ms;
            res->pi0_result.action_dim = slot.pi0_result.action_dim;
            res->pi0_result.action_steps = slot.pi0_result.action_steps;
            res->pi0_result.has_action = slot.pi0_result.has_action;
            if (slot.pi0_result.has_action && slot.pi0_result.action_data != nullptr) {
                const int64_t action_count = (int64_t) slot.pi0_result.action_dim * slot.pi0_result.action_steps;
                res->pi0_result.action.assign(slot.pi0_result.action_data, slot.pi0_result.action_data + action_count);
            }
        }
            ...
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                    ...
                    if (slot.state == SLOT_STATE_STARTED) {
                        ...
                        // pi0 requests must always re-process the full prompt
                        // (including image chunks) through mtmd_helper_eval_chunks_pi0(),
                        // because pi0 is a policy model whose inference result depends on
                        // the complete forward pass each time.  Even when the text prompt
                        // is identical between requests, the robot state / image may differ,
                        // so prompt-cache reuse is unsafe �?force n_past to 0 to guarantee
                        // the full encode/decode pipeline runs every time.
                        if (slot.task->params.is_pi0 && n_past > 0) {
                            SLT_WRN(slot, "pi0 mode: disabling prompt cache reuse, forcing full re-processing (was n_past = %d)\n", n_past);
                            n_past = 0;
                        }
                    ...
                    if (slot.prompt.n_tokens() < slot.task->n_tokens() && input_tokens[slot.prompt.n_tokens()] == LLAMA_TOKEN_NULL) {
                        // process the image; pass is_pi0 so the correct eval function is used
                        size_t n_tokens_out = 0;
                        int32_t res = input_tokens.process_chunk(ctx, mctx, slot.prompt.n_tokens(), slot.prompt.tokens.pos_next(), slot.id, n_tokens_out, slot.task->params.is_pi0, &slot.pi0_result);
                        if (res != 0) {
                            SLT_ERR(slot, "failed to process image, res = %d\n", res);
                            send_error(slot, slot.task->params.is_pi0 ? "failed to process pi0 media chunks" : "failed to process image", ERROR_TYPE_SERVER);
                            slot.release();
                            continue;
                        }
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_DONE_PROMPT) {
                    ...
                    if (slot.task->params.is_pi0) {
                        // pi0 requests follow the CLI semantics: after
                        // mtmd_helper_eval_chunks_pi0() finishes the forward pass,
                        // the request is complete and must not enter the standard
                        // text generation / sampling loop.
                        slot.stop = STOP_TYPE_EOS;
                        slot.n_decoded = 0;
...
static std::vector<float> string_to_float_array(const std::string & input) {
...
void server_routes::init_routes() {
    ...
    this->post_foreground_image = [this](const server_http_req & req) {
        auto res = std::make_unique<server_res_generator>(ctx_server);
        if (ctx_server.mctx == nullptr || !mtmd_support_vision(ctx_server.mctx)) {
            res->error(format_error_response("foreground image is not supported without multimodal vision support", ERROR_TYPE_NOT_SUPPORTED));
            return res;
        }
        ...
        if (path.empty()) {
            res->error(format_error_response("\"path\" is required", ERROR_TYPE_INVALID_REQUEST));
            return res;
        }

        std::lock_guard<std::mutex> lock(ctx_server.foreground.mutex);
        if (!ctx_server.foreground_load_media(path)) {
            res->error(format_error_response("failed to load media", ERROR_TYPE_INVALID_REQUEST));
            return res;
        }

        ctx_server.foreground.content += mtmd_default_marker();
        res->ok({
            {"loaded", true},
            {"pending_text", ctx_server.foreground.content},
    ...
    this->put_foreground_state = [this](const server_http_req & req) {
        ...
        std::lock_guard<std::mutex> lock(ctx_server.foreground.mutex);
        const std::vector<float> state_array = string_to_float_array(state);
        llama_set_pi0_state(ctx_server.ctx, state_array.data(), state_array.size());
    ...
    this->post_foreground_infer = [this](const server_http_req & req) {
        auto res = std::make_unique<server_res_generator>(ctx_server);
        if (ctx_server.mctx == nullptr) {
            res->error(format_error_response("foreground infer requires multimodal server context", ERROR_TYPE_NOT_SUPPORTED));
            return res;
        }

        const json body = json::parse(req.body);
        const pi_model_kind pi_model = server_pi_model_kind();
        const bool has_pi05_payload = server_request_has_pi05_payload(body);
        {
            std::ostringstream oss;
            oss << "foreground_infer pi_model=" << pi_model_kind_name(pi_model)
                << " use_pi05_adapter=" << (server_pi_model_use_pi05_adapters() ? 1 : 0)
                << " has_pi05_payload=" << (has_pi05_payload ? 1 : 0)
                << " body_keys=" << body.size();
            server_pi_model_debug_dump_line(oss.str());
        }
        if (has_pi05_payload && pi_model_kind_is_pi0(pi_model)) {
            res->error(format_error_response("PI_MODEL=pi0 does not accept pi05 base64 foreground payload", ERROR_TYPE_INVALID_REQUEST));
            return res;
        }

        if (has_pi05_payload && server_pi_model_use_pi05_adapters()) {
            const int64_t t_handler_start = ggml_time_us();
            server_context_impl::pi0_http_infer_request pi0_req;
            try {
                pi0_req = server_context_impl::parse_pi0_http_infer_request(body);
            } catch (const std::exception & err) {
                res->error(format_error_response(err.what(), ERROR_TYPE_INVALID_REQUEST));
                return res;
            }
            const int64_t t_parse_done = ggml_time_us();

            try {
                std::lock_guard<std::mutex> lock(ctx_server.foreground.mutex);
                ctx_server.foreground.chat_history.clear();
                ctx_server.foreground.bitmaps.entries.clear();
                ctx_server.foreground.content.clear();
                ctx_server.foreground.n_past = 0;
                common_sampler_reset(ctx_server.foreground.smpl);

                ctx_server.foreground.latest_state = pi0_req.state;
                llama_set_pi0_state(ctx_server.ctx, pi0_req.state.data(), pi0_req.state.size());
                ctx_server.foreground.state_flag = true;

                mtmd::bitmap head = server_context_impl::decode_pi0_image_b64(
                        ctx_server.mctx, pi0_req, pi0_req.head_image_b64);
                mtmd::bitmap left = server_context_impl::decode_pi0_image_b64(
                        ctx_server.mctx, pi0_req, pi0_req.left_image_b64);
                mtmd::bitmap right = server_context_impl::decode_pi0_image_b64(
                        ctx_server.mctx, pi0_req, pi0_req.right_image_b64);
                if (!head.ptr || !left.ptr || !right.ptr) {
                    res->error(format_error_response("failed to decode one or more input images", ERROR_TYPE_INVALID_REQUEST));
                    return res;
                }

                ctx_server.foreground.bitmaps.entries.push_back(std::move(head));
                ctx_server.foreground.bitmaps.entries.push_back(std::move(left));
                ctx_server.foreground.bitmaps.entries.push_back(std::move(right));
                const std::string pi05_prompt =
                        build_pi05_openpi_prompt(pi0_req.prompt, pi0_req.state);

                ctx_server.foreground.content =
                ...
                common_chat_msg msg;
                msg.role = "user";
                msg.content = ctx_server.foreground.content;

                json eval_timing = json::object();
                const int64_t t_eval_start = ggml_time_us();
                const int ret = ctx_server.foreground_eval_message(msg, /* is_pi0 = */ true, &eval_timing);
                const int64_t t_eval_done = ggml_time_us();
                if (ret != 0) {
                    res->error(format_error_response("failed to eval pi0 request", ERROR_TYPE_SERVER));
                    return res;
                }

                const int64_t t_response_build_start = ggml_time_us();
                json response = {
                    {"prompt", pi0_req.prompt},
                    {"state", ctx_server.foreground.latest_state},
                    {"action", server_context_impl::reshape_action_rows(ctx_server.foreground.latest_pi0_result, false)},
                    {"action_final", server_context_impl::reshape_action_rows(ctx_server.foreground.latest_pi0_result, true)},
                    {"action_final_raw", server_context_impl::reshape_action_raw_rows(ctx_server.foreground.latest_pi0_result, true)},
                    {"joint_command", server_context_impl::slice_joint_command_rows(ctx_server.foreground.latest_pi0_result, false)},
                    {"joint_command_final", server_context_impl::slice_joint_command_rows(ctx_server.foreground.latest_pi0_result, true)},
                    {"pi0_action_norm_debug", pi0_action_norm_debug_json()},
                    {"action_steps", ctx_server.foreground.latest_pi0_result.action_steps},
                    {"action_dim", ctx_server.foreground.latest_pi0_result.action_dim},
                    {"is_pi0", true},
                    {"pi_model", pi_model_kind_name(pi_model)}
                };
                if (ctx_server.foreground.latest_pi0_result.has_vit_ms) {
                    response["vit_ms"] = ctx_server.foreground.latest_pi0_result.vit_ms;
                }
                if (ctx_server.foreground.latest_pi0_result.has_encode_ms) {
                    response["encode_ms"] = ctx_server.foreground.latest_pi0_result.encode_ms;
                }
                if (ctx_server.foreground.latest_pi0_result.has_decode_ms) {
                    response["decode_ms"] = ctx_server.foreground.latest_pi0_result.decode_ms;
                }
                if (ctx_server.foreground.latest_pi0_result.has_total_ms) {
                    response["total_ms"] = ctx_server.foreground.latest_pi0_result.total_ms;
                }
                if (ctx_server.foreground.latest_pi0_result.has_batch_build_ms) {
                    response["batch_build_ms"] = ctx_server.foreground.latest_pi0_result.batch_build_ms;
                }
                if (ctx_server.foreground.latest_pi0_result.has_output_extract_ms) {
                    response["output_extract_ms"] = ctx_server.foreground.latest_pi0_result.output_extract_ms;
                }
                if (ctx_server.foreground.latest_pi0_result.has_batch_free_ms) {
                    response["batch_free_ms"] = ctx_server.foreground.latest_pi0_result.batch_free_ms;
                }

                json timing = {
                    {"handler_ms", (ggml_time_us() - t_handler_start) / 1000.0},
                    {"parse_ms", (t_parse_done - t_handler_start) / 1000.0},
                    {"eval_ms", (t_eval_done - t_eval_start) / 1000.0},
                    {"response_build_ms", (ggml_time_us() - t_response_build_start) / 1000.0}
                };
        ...
        if (is_pi0) {
            text = server_pi_model_use_pi05_adapters() ? build_pi05_openpi_prompt(
                    raw_text,
                    ctx_server.foreground.latest_state) : raw_text;
        }

        ctx_server.foreground.content += text;
        ...
        const int64_t t_eval_start = ggml_time_us();
        json eval_timing = json::object();
        const int ret = ctx_server.foreground_eval_message(msg, is_pi0, &eval_timing);
        const int64_t t_eval_done = ggml_time_us();
        if (ret != 0) {
            res->error(format_error_response("failed to eval prompt", ERROR_TYPE_SERVER));
            return res;
        }

        std::string generated_text;
        int64_t t_generate_done = t_eval_done;
        if (!is_pi0) {
            if (ctx_server.foreground_generate_response(n_predict, generated_text) != 0) {
                res->error(format_error_response("failed to generate response", ERROR_TYPE_SERVER));
                return res;
            }
            t_generate_done = ggml_time_us();
        }
        ...
        const int64_t t_response_build_done = ggml_time_us();
        response["timing_breakdown_ms"] = {
            {"request_parse_ms",       (t_parse_done - t_handler_start) / 1000.0},
            {"pre_eval_ms",            (t_eval_start - t_parse_done) / 1000.0},
            {"eval_ms",                (t_eval_done - t_eval_start) / 1000.0},
            {"generate_ms",            (t_generate_done - t_eval_done) / 1000.0},
            {"response_build_ms",      (t_response_build_done - t_response_build_start) / 1000.0},
            {"handler_total_ms",       (t_response_build_done - t_handler_start) / 1000.0},
    ...
    this->post_foreground_reset = [this](const server_http_req &) {
        auto res = std::make_unique<server_res_generator>(ctx_server);
        std::lock_guard<std::mutex> lock(ctx_server.foreground.mutex);
        ctx_server.foreground_reset();
        res->ok({{"reset", true}});
        return res;
    };

    this->get_foreground_session = [this](const server_http_req &) {
        auto res = std::make_unique<server_res_generator>(ctx_server);
        std::lock_guard<std::mutex> lock(ctx_server.foreground.mutex);
        res->ok({
            {"pending_text", ctx_server.foreground.content},

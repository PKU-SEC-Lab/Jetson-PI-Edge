...
struct clip_graph_siglip : clip_graph {
    ...
    // The original PI05 path evaluates all camera views in one ViT graph.
    // Other SigLIP projectors keep their upstream batching policy.
    bool support_batch() const override { return proj_type == PROJECTOR_TYPE_PI0; }

private:
    ggml_tensor * build_vit_pi0_legacy(ggml_tensor * inp);
    ggml_tensor * build_vit_pi05_legacy(ggml_tensor * inp);
};

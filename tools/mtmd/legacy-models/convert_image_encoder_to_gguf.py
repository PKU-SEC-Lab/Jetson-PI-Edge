...
ap.add_argument("--projector-type", help="Type of projector. Possible values: mlp, ldp, ldpv2,pi0", default="mlp")
...
use_gelu = v_hparams["hidden_act"] in {
    "gelu",
    "gelu_fast",
    "gelu_new",
    "gelu_pytorch_tanh",
...
# def normalize_vision_state_dict_key(name: str) -> str:
#     # SiglipVisionModel.state_dict() may omit the "vision_model." wrapper prefix
#     # (same issue handled in minicpmv-convert-image-encoder-to-gguf.py).
#     if name.startswith("vision_tower."):
#         return "vision_model." + name[len("vision_tower."):]
...
# def ensure_vision_gguf_prefix(name: str) -> str:
#     if name.startswith("v."):
#         return name
#     if name.startswith(("blk.", "patch_embd.", "position_embd.", "post_ln.")):
#         return "v." + name
#     return name
...
#     name = normalize_vision_state_dict_key(name)
#     name = ensure_vision_gguf_prefix(get_tensor_name(name))
#     data = data.detach().cpu()
#     if data.dtype == torch.bfloat16:
#         data = data.float()
#     data = data.squeeze().numpy()

...
#         if k.startswith("paligemma_with_expert.paligemma.model."):
#             del checkpoint[k]
#             checkpoint[k.replace("paligemma_with_expert.paligemma.model.",'')] = v
...
def rename_checkpoint(checkpoint):
    ...
    keys_to_rename = [
        ...
        if any(k.startswith(prefix) for prefix in paligemma_prefixes)
    ]
    
    for k in keys_to_rename:
        prefix = next(prefix for prefix in paligemma_prefixes if k.startswith(prefix))
        checkpoint[k[len(prefix):]] = checkpoint.pop(k)

    outer_model_keys = [k for k in checkpoint.keys() if k.startswith("model.")]
    for k in outer_model_keys:
        new_key = k[len("model."):]
        if new_key in checkpoint:
            raise ValueError(f"Refusing to overwrite duplicate tensor key: {new_key}")
        checkpoint[new_key] = checkpoint.pop(k)
    
    # Optional debug log.
    if keys_to_rename or outer_model_keys:
        print(
            f"Renamed {len(keys_to_rename) + len(outer_model_keys)} tensors "
            "for Pi0/PaliGemma format."
        )
...
if newline_checkpoint_path is not None:
    ...
    rename_checkpoint(first_checkpoint)
...
if projector_checkpoint_path is not None:
    ...
    rename_checkpoint(last_checkpoint)

...
def rename_checkpoint(checkpoint):
    # 1. Find keys that need to be renamed.
    keys_to_rename = [k for k in checkpoint.keys() if k.startswith("paligemma_with_expert.paligemma.model.")]
    
    # 2. Rename them in one pass.
    for k in keys_to_rename:
        new_key = k.replace("paligemma_with_expert.paligemma.model.", "")
        checkpoint[new_key] = checkpoint.pop(k) # pop returns and removes the old value safely
    
    # Optional debug log.
    if keys_to_rename:
        print(f"Renamed {len(keys_to_rename)} tensors for Pi0/PaliGemma format.")

...
#include "clip_vit_debug_print.h"
...
using clip_perf_clock = std::chrono::high_resolution_clock;

static double clip_perf_elapsed_ms(const clip_perf_clock::time_point & t0, const clip_perf_clock::time_point & t1) {
    return std::chrono::duration<double, std::milli>(t1 - t0).count();
}

static bool clip_pi0_vit_perf_enabled() {
    static const bool enabled = []() {
        const char * env = std::getenv("LLAMA_PI0_VIT_PERF");
        return env != nullptr && env[0] != '\0' && std::atoi(env) != 0;
    }();
    return enabled;
}

static bool clip_pi_model_use_pi05_vit_adapter() {
    return pi_model_use_pi05_adapters_by_default(pi_model_kind_from_env()) != 0;
}

static bool clip_pi0_graph_reuse_disabled() {
    static const bool disabled = []() {
        const char * env = std::getenv("CLIP_PI0_VIT_GRAPH_REUSE_DISABLE");
        return env != nullptr && env[0] != '\0' && std::atoi(env) != 0;
    }();
    return disabled;
}

static std::string pi05_clip_debug_base_tensor_name(const char * name) {
    if (name == nullptr) {
        return "";
    }
    std::string tensor_name(name);
    const size_t paren_pos = tensor_name.find(" (");
    if (paren_pos != std::string::npos) {
        tensor_name.resize(paren_pos);
    }
    while (!tensor_name.empty() && std::isspace((unsigned char) tensor_name.back())) {
        tensor_name.pop_back();
    }
    const size_t dash_pos = tensor_name.rfind('-');
...
static bool pi05_clip_debug_should_keep_tensor(const char * name) {
    const std::string tensor_name = pi05_clip_debug_base_tensor_name(name);
    return tensor_name.find("pi05_dbg_siglip_") == 0;
}

static bool pi05_clip_debug_enabled() {
    return pi_model_debug_dump_enabled();
}

static bool pi05_clip_debug_siglip_layer_enabled(int il) {
    const char * raw = std::getenv("PI05_DEBUG_SIGLIP_LAYERS");
    std::string layers = raw != nullptr && raw[0] != '\0' ? raw : "0";
    for (char & ch : layers) {
...
static bool pi05_clip_quant_debug_enabled() {
    const char * raw = std::getenv("PI05_DEBUG_QUANT");
    return raw != nullptr && raw[0] != '\0' && std::strcmp(raw, "0") != 0;
}

static ggml_tensor * pi05_clip_preserve_quantized_weight(ggml_context * ctx0, ggml_tensor * w) {
    if (w == nullptr || ggml_is_quantized(w->type)) {
        return w;
    }
    return ggml_cast(ctx0, ggml_cast(ctx0, w, GGML_TYPE_BF16), GGML_TYPE_F32);
}

static std::string pi05_clip_debug_siglip_name(int il, const char * suffix) {
    return "pi05_dbg_siglip_l" + std::to_string(il) + "_" + suffix;
}

static size_t pi05_clip_debug_preview_count(size_t n) {
    if (n == 0) {
        return 0;
    }
    return std::min(n, (size_t) pi_model_debug_dump_values(32));
}

static void pi05_clip_debug_dump_host_vector(
        const char * name,
    ...
    const size_t preview_count = pi05_clip_debug_preview_count(values.size());
...
static void pi05_clip_debug_dump_tensor_preview(std::ofstream & ofs, ggml_tensor * tensor) {
    if (tensor == nullptr) {
        return;
    }
    ...
    if (tensor->type != GGML_TYPE_F32) {
        ofs << "preview_skipped_non_f32\n";
        return;
    }

    const size_t preview_count = pi05_clip_debug_preview_count((size_t) n);
    std::vector<float> values(preview_count);
    ggml_backend_tensor_get(tensor, values.data(), 0, preview_count * sizeof(float));
...
static void pi05_clip_debug_dump_graph_tensors_to_file(ggml_backend_sched_t sched, ggml_cgraph * gf) {
    if (!pi05_clip_debug_enabled()) {
        return;
    }
    const char * dump_path = pi_model_debug_dump_file();
    ...
    int matched = 0;
    ofs << "===== pi05 siglip tensor dump begin =====\n";
    ofs << "gf_n_nodes=" << ggml_graph_n_nodes(gf) << "\n";
    for (int i = 0; i < ggml_graph_n_nodes(gf); ++i) {
        ggml_tensor * node = ggml_graph_node(gf, i);
        if (node == nullptr || !pi05_clip_debug_should_keep_tensor(node->name)) {
            continue;
        }
        ++matched;
        ...
        ofs << "node_index=" << i
            ...
            << "\n";
        pi05_clip_debug_dump_tensor_preview(ofs, node);
    }
    ofs << "matched_nodes=" << matched << "\n";
    ofs << "===== pi05 siglip tensor dump end =====\n";
    ofs.flush();
}
...
struct clip_model {
    ...
    ggml_tensor * mm_w = nullptr;
    ggml_tensor * mm_b = nullptr;
...
struct clip_ctx {
    ...
    // PI0 VIT graph cache (fixed resolution reuse, see pi0_vit_optimization.md)
    ggml_context_ptr pi0_ctx0_ptr;
    ggml_cgraph *     pi0_gf           = nullptr;
    int               pi0_cached_nx    = -1;
    int               pi0_cached_ny    = -1;
    std::vector<int32_t> pi0_positions_buf;
    clip_pi0_vit_perf    pi0_last_vit_perf;
    ...
    clip_ctx(clip_context_params & ctx_params) {
        ...
        if (ctx_params.use_gpu) {
            if (ctx_params.device != nullptr) {
                ...
                if (!backend) {
                    throw std::runtime_error("failed to initialize requested backend device");
                }
            } else if (auto backend_name = std::getenv("MTMD_BACKEND_DEVICE")) {
...
void clip_pi0_invalidate_graph_cache(clip_ctx * ctx) {
    if (ctx == nullptr) {
        return;
    }
    ctx->pi0_ctx0_ptr.reset();
    ctx->pi0_gf           = nullptr;
    ctx->pi0_cached_nx    = -1;
    ctx->pi0_cached_ny    = -1;
    ctx->pi0_positions_buf.clear();
}

const clip_pi0_vit_perf * clip_get_last_pi0_vit_perf(const clip_ctx * ctx) {
    return ctx != nullptr ? &ctx->pi0_last_vit_perf : nullptr;
}
...
struct clip_graph {
    ...
    ggml_cgraph * build_pi0() {

        ggml_tensor * inp = build_inp();
        cb(inp, "pi05_dbg_siglip_patch_tokens", -1);
        ggml_tensor * learned_pos_embd = model.position_embeddings;

        const int n_tokens = inp->ne[1];
        // 2. Create the positions tensor with I32 type.
        struct ggml_tensor * positions_data = build_positions_tensor(n_tokens);

        ggml_tensor * embeddings = ggml_get_rows(ctx0, learned_pos_embd, positions_data);
        ggml_tensor * pos_emb = (embeddings->type == GGML_TYPE_F16)
            ? ggml_cast(ctx0, embeddings, GGML_TYPE_F32)
            : embeddings;
        
        ggml_tensor * inp2 = ggml_add(ctx0, inp, pos_emb);
        cb(inp2, "pi05_dbg_siglip_pos_tokens", -1);
        // openpi casts SigLIP patch + position tokens to dtype_mm (bf16) before the ViT encoder.
        // inp2 = ggml_cast(ctx0, ggml_cast(ctx0, inp2, GGML_TYPE_BF16), GGML_TYPE_F32);
        // cb(inp2, "pi05_dbg_siglip_encoder_input", -1);


        const bool use_pi05_vit_adapter = clip_pi_model_use_pi05_vit_adapter();
        ggml_tensor * cur = use_pi05_vit_adapter
            ? build_vit_pi0(
                    inp2, n_patches,
                    NORM_TYPE_NORMAL,
                    hparams.ffn_op,
                    learned_pos_embd,
                    nullptr)
            : build_vit_pi0_legacy(
                    inp2, n_patches,
                    NORM_TYPE_NORMAL,
                    hparams.ffn_op,
                    learned_pos_embd,
                    nullptr);
        cb(cur, "pi05_dbg_siglip_pre_mmproj", -1);

        if (use_pi05_vit_adapter) {
            cur = build_dense_pi0_bf16(model.mm_w, model.mm_b, cur);
        } else {
            cur = ggml_mul_mat(ctx0, model.mm_w, cur);
            cur = ggml_add(ctx0, cur, model.mm_b);
        }
        cb(cur, "pi05_dbg_siglip_mmproj_out", -1);
    ...
    // P0 optimization: batched ViT for multiple images
    ggml_cgraph * build_pi0_batched(int n_images) {
        ggml_tensor * inp_raw = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, img.nx, img.ny, 3 * n_images);
        ...
        for (int i = 0; i < n_images; i++) {
            ggml_tensor * img_i = ggml_view_3d(ctx0, inp_raw,
                img.nx, img.ny, 3,
                inp_raw->nb[1], inp_raw->nb[2],
                (int64_t)i * 3 * inp_raw->nb[2]);

            ggml_tensor * patches_i = ggml_conv_2d(ctx0, patch_kernel, img_i,
                patch_size, patch_size, 0, 0, 1, 1);
            patches_i = ggml_reshape_2d(ctx0, patches_i, n_patches, n_embd);
            patches_i = ggml_cont(ctx0, ggml_transpose(ctx0, patches_i));
            patches_i = ggml_add(ctx0, patches_i, model.patch_bias);
            patch_list.push_back(patches_i);
        }

        ggml_tensor * inp;
        if (n_images == 1) {
            inp = ggml_reshape_3d(ctx0, patch_list[0], patch_list[0]->ne[0], patch_list[0]->ne[1], 1);
        } else {
            inp = ggml_reshape_3d(ctx0, patch_list[0], patch_list[0]->ne[0], patch_list[0]->ne[1], 1);
            for (int i = 1; i < n_images; i++) {
                ggml_tensor * p = ggml_reshape_3d(ctx0, patch_list[i], patch_list[i]->ne[0], patch_list[i]->ne[1], 1);
                inp = ggml_concat(ctx0, inp, p, 2);
            }
        }

        ggml_tensor * learned_pos_embd = model.position_embeddings;
        const int n_tokens_per_img = n_patches;
        struct ggml_tensor * positions_data = build_positions_tensor(n_tokens_per_img);
        ggml_tensor * embeddings_pos = ggml_get_rows(ctx0, learned_pos_embd, positions_data);
        ggml_tensor * pos_emb = (embeddings_pos->type == GGML_TYPE_F16)
            ? ggml_cast(ctx0, embeddings_pos, GGML_TYPE_F32)
            : embeddings_pos;
        ggml_tensor * inp2 = ggml_add(ctx0, inp, pos_emb);

        ggml_tensor * inpL = inp2;
        for (int il = 0; il < n_layer; il++) {
            auto & layer = model.layers[il];
            ggml_tensor * cur = inpL;

            cur = build_norm_pi0_bf16(cur, layer.ln_1_w, layer.ln_1_b, NORM_TYPE_NORMAL, eps, il);

            {
                ggml_tensor * Qcur = build_dense_pi0_bf16(layer.q_w, layer.q_b, cur);
                ggml_tensor * Kcur = build_dense_pi0_bf16(layer.k_w, layer.k_b, cur);
                ggml_tensor * Vcur = build_dense_pi0_bf16(layer.v_w, layer.v_b, cur);

                Qcur = ggml_reshape_4d(ctx0, Qcur, d_head, n_head, n_tokens_per_img, n_images);
                Kcur = ggml_reshape_4d(ctx0, Kcur, d_head, n_head, n_tokens_per_img, n_images);
                Vcur = ggml_reshape_4d(ctx0, Vcur, d_head, n_head, n_tokens_per_img, n_images);
                ...
                ggml_tensor * q_scaled_attn = ggml_permute(ctx0, q_scaled, 0, 2, 1, 3);
                ggml_tensor * kq = ggml_mul_mat(ctx0, k, q_scaled_attn);
                kq = ggml_soft_max_ext(ctx0, kq, nullptr, 1.0f, 0.0f);

                ggml_tensor * kqv = ggml_mul_mat(ctx0, v, kq);
                ggml_tensor * kqv_bthd = ggml_permute(ctx0, kqv, 0, 2, 1, 3);
                ggml_tensor * attn_out = ggml_cont_3d(ctx0, kqv_bthd,
                    kqv_bthd->ne[0] * n_head, n_tokens_per_img, n_images);

                cur = ggml_mul_mat(ctx0, layer.o_w, attn_out);
                if (layer.o_b) {
            ...
            cur = build_norm_pi0_bf16(cur, layer.ln_2_w, layer.ln_2_b, NORM_TYPE_NORMAL, eps, il);
            cur = build_ffn_pi0(cur,
        ...
        inpL = build_norm_pi0_bf16(inpL, model.post_ln_w, model.post_ln_b, NORM_TYPE_NORMAL, eps, -1);
        ggml_tensor * cur = build_dense_pi0_bf16(model.mm_w, model.mm_b, inpL);
    ...
    ggml_tensor * build_positions_tensor(int n_tokens) {
        ggml_tensor * positions_data = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_tokens);

        ggml_set_name(positions_data, "positions_data");
        ggml_set_input(positions_data);
        return positions_data;
    }

    ggml_tensor * build_ones(int n_dim,int n_tokens) const {
        ggml_tensor * ones = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_dim, n_tokens);
...
private:
    ...
    void cb(ggml_tensor * cur0, const char * name, int il) const {
        if (ctx->debug_graph || pi05_clip_debug_should_keep_tensor(name)) {
    ...
    ggml_tensor * build_ffn_pi0_legacy(
        ggml_tensor * cur,
        ggml_tensor * up,
        ggml_tensor * up_b,
        ggml_tensor * down,
        ...
        ggml_tensor * tmp = ggml_mul_mat(ctx0, up, cur);
        cb(tmp, "ffn_up", il);

        tmp = ggml_cont(ctx0, tmp);
        ggml_tensor * up_b_aligned = ggml_cont(ctx0, up_b);
        tmp = ggml_add(ctx0, tmp, up_b_aligned);
        cb(tmp, "ffn_up_b", il);
        ...
        cb(cur, "ffn_down", il);
        cur = ggml_cont(ctx0, cur);
        ggml_tensor * down_b_aligned = ggml_cont(ctx0, down_b);
        cur = ggml_add(ctx0, cur, down_b_aligned);

        return ggml_cont(ctx0, cur);
    }

    ggml_tensor * build_vit_pi0_legacy(
        ggml_tensor * inp,
        int64_t n_pos,
        norm_type norm_t,
        ffn_op_type ffn_t,
    ...
    ggml_tensor * build_vit_pi0(
        ...
        // loop over layers
        for (int il = 0; il < n_layer; il++) {
            const bool debug_siglip_layer = pi05_clip_debug_siglip_layer_enabled(il);
            auto & layer = model.layers[il];
            ggml_tensor * cur = inpL; // inpL = residual, cur = hidden_states
            
            // layernorm1
            cur = build_norm_pi0_bf16(cur, layer.ln_1_w, layer.ln_1_b, norm_t, eps, il);
            cb(cur, "layer_inp_normed", il);
            // self-attention
            {
                ggml_tensor * Qcur = build_dense_pi0_bf16(layer.q_w, layer.q_b, cur);
                ...
                ggml_tensor * Vcur = build_dense_pi0_bf16(layer.v_w, layer.v_b, cur);
                if (debug_siglip_layer) {
                    cb(cur, pi05_clip_debug_siglip_name(il, "attn_norm").c_str(), -1);
                    cb(Qcur, pi05_clip_debug_siglip_name(il, "q_mm").c_str(), -1);
                    cb(Kcur, pi05_clip_debug_siglip_name(il, "k_mm").c_str(), -1);
                    cb(Vcur, pi05_clip_debug_siglip_name(il, "v_mm").c_str(), -1);
                }
                ...
                cur = build_attn(layer.o_w, layer.o_b,
                    Qcur, Kcur, Vcur, nullptr, kq_scale, il, true);
                // cur = build_bf16_roundtrip(cur);

                cb(cur, "attn_out", il);
                if (debug_siglip_layer) {
                    cb(cur, pi05_clip_debug_siglip_name(il, "attn_out").c_str(), -1);
                }
            }
            ...
            cb(cur, "ffn_inp", il);
            if (debug_siglip_layer) {
                cb(cur, pi05_clip_debug_siglip_name(il, "attn_residual").c_str(), -1);
            }

            // layernorm2
            cur = build_norm_pi0_bf16(cur, layer.ln_2_w, layer.ln_2_b, norm_t, eps, il);
            cb(cur, "ffn_inp_normed", il);
            // ffn
            cur = build_ffn_pi0(cur,
                layer.ff_up_w, layer.ff_up_b,
            ...
            cb(cur, "ffn_out", il);
            if (debug_siglip_layer) {
                cb(cur, pi05_clip_debug_siglip_name(il, "ffn_out").c_str(), -1);
            }
            ...
            cb(cur, "layer_out", il);
            if (debug_siglip_layer) {
                cb(cur, pi05_clip_debug_siglip_name(il, "ffn_residual").c_str(), -1);
            }
        ...
        // post-layernorm
        inpL = build_norm_pi0_bf16(inpL, model.post_ln_w, model.post_ln_b, norm_t, eps, -1);
    ...
    ggml_tensor * build_inp() {
        ...
        ggml_tensor * patch_kernel = (model.patch_embeddings_0->type == GGML_TYPE_F16)
            ? ggml_cast(ctx0, model.patch_embeddings_0, GGML_TYPE_F32)
            : model.patch_embeddings_0;
        ggml_tensor * inp = ggml_conv_2d(ctx0, patch_kernel, inp_raw, patch_size, patch_size, 0, 0, 1, 1);
        // ggml_tensor * inp = ggml_conv_2d(ctx0, model.patch_embeddings_0, inp_raw, patch_size, patch_size, 0, 0, 1, 1);
    ...
    ggml_tensor * build_bf16_roundtrip(ggml_tensor * cur) const {
        if (cur == nullptr) {
            return nullptr;
        }
        return ggml_cast(ctx0, ggml_cast(ctx0, cur, GGML_TYPE_BF16), GGML_TYPE_F32);
    }

    ggml_tensor * build_norm_pi0_bf16(
            ggml_tensor * cur,
            ggml_tensor * mw,
            ggml_tensor * mb,
            norm_type type,
        ...
        cur = build_norm(
            cur,
            // build_bf16_roundtrip(mw),
            // build_bf16_roundtrip(mb),
            mw,
            mb,
            type,
            norm_eps,
            il);
        // return build_bf16_roundtrip(cur);
        return cur;
    }

    ggml_tensor * build_dense_pi0_bf16(
            ggml_tensor * w,
            ggml_tensor * b,
            ggml_tensor * cur) const {
        // if (pi05_clip_quant_debug_enabled()) {
        //     LOG_INF("%s: dense weight=%s type=%s quantized=%d cur_type=%s ne=[%" PRId64 ", %" PRId64 ", %" PRId64 ", %" PRId64 "]\n",
        //         __func__, w ? w->name : "<null>", w ? ggml_type_name(w->type) : "<null>", w ? ggml_is_quantized(w->type) : 0,
        //         cur ? ggml_type_name(cur->type) : "<null>",
        //         w ? w->ne[0] : 0, w ? w->ne[1] : 0, w ? w->ne[2] : 0, w ? w->ne[3] : 0);
        // }

        // cur = build_bf16_roundtrip(cur);
        // cur = ggml_mul_mat(ctx0, pi05_clip_preserve_quantized_weight(ctx0, w), cur);
        cur = ggml_mul_mat(ctx0, w, cur);
        if (b) {
            // cur = ggml_add(ctx0, cur, build_bf16_roundtrip(b));
    ...
    //     ggml_tensor * tmp = up ? ggml_mul_mat(ctx0, up, cur) : cur;
    //     cur = ggml_cont(ctx0, tmp);
    //     cb(tmp, "ffn_up", il);
    //     struct ggml_tensor * up_b_aligned = ggml_cont(ctx0, up_b);
    //     if (up_b) {
    //         tmp = ggml_add(ctx0, tmp, up_b_aligned);
    //         cb(tmp, "ffn_up_b", il);
    //     }
    ...
    //         cb(cur, "ffn_down", il);
    //     }
    //     struct ggml_tensor *down_b_aligned = ggml_cont(ctx0, down_b);
    //     if (down_b) {
    //         cur = ggml_add(ctx0, cur, down_b_aligned);
    //     }
    //     cur = ggml_cont(ctx0, cur);
    ...
    ggml_tensor* ggml_gelu_tanh(ggml_context* ctx, ggml_tensor* x) {
        // Constants.
        ...
        ggml_tensor* ones = build_ones(n_dim,n_tokens);
        // struct ggml_tensor * ones = ggml_new_tensor_4d(ctx0, inp->type, ne0, ne1, ne2, ne3);
        // ggml_set_f32(ones, 1.0f);
        
        // Compute x^3.
        ggml_tensor* x3 = ggml_mul(ctx, 
                                   ggml_mul(ctx, x, x), // x^2
                                   x);                  // * x = x^3
        
        // Compute the inner term: x + 0.044715 * x^3.
        ggml_tensor* inner = ggml_add(ctx, 
                                      x, 
                                      ggml_scale(ctx, x3, coef)); // scale is a convenient multiplication helper.
        
        // Compute tanh input: sqrt(2/pi) * inner.
        ggml_tensor* tanh_param = ggml_scale(ctx, inner, sqrt_2_over_pi);
        
        // Compute tanh(...).
        ggml_tensor* tanh_val = ggml_tanh(ctx, tanh_param);
        
        // Compute 1 + tanh(...).
        // ggml_tensor* one_plus_tanh = ggml_add_constant(ctx0, tanh_val, 1.0f); // scalar add.
        ggml_tensor* one_plus_tanh = ggml_add(ctx0, tanh_val, ones); 
        
        // Final expression: 0.5 * x * (1 + tanh(...)).
        ggml_tensor* half_x = ggml_scale(ctx, x, 0.5f);
        ggml_tensor* result = ggml_mul(ctx, half_x, one_plus_tanh);
    ...
    ggml_tensor * build_ffn_pi0(
        ...
        // 1. Up-projection
        ggml_tensor * tmp = build_dense_pi0_bf16(up, up_b, cur);
        ...
        // 4. Down-projection
            ...
            // mul_mat is sensitive to the memory continuity of cur.
        cur = ggml_cont(ctx0, cur);
        cur = build_dense_pi0_bf16(down, down_b, cur);
        

        // 5. Down-bias, another 4304-dimensional add.

        cb(cur, "ffn_down", il);
        
        
        // Keep the final output contiguous before the later residual add.
        return ggml_cont(ctx0, cur);
    }
    ...
    ggml_tensor * build_attn(
            ...
            float kq_scale,
            int il,
            bool pi0_bf16_dense = false) const {
        ...
        const bool use_flash_attn = ctx->flash_attn_type == CLIP_FLASH_ATTN_TYPE_ENABLED && !pi0_bf16_dense;
        const bool debug_siglip_layer = pi0_bf16_dense && pi05_clip_debug_siglip_layer_enabled(il);
        if (use_flash_attn) {
            ...
            v = ggml_cont(ctx0, v);
            if (debug_siglip_layer) {
                cb(v, pi05_clip_debug_siglip_name(il, "v_attn").c_str(), -1);
            }
            ...
            if (pi0_bf16_dense) {
                // ggml_tensor * q_scaled = build_bf16_roundtrip(ggml_scale(ctx0, q_cur, kq_scale));
                ggml_tensor * q_scaled = ggml_scale(ctx0, q_cur, kq_scale);
                ggml_tensor * q_scaled_attn = ggml_permute(ctx0, q_scaled, 0, 2, 1, 3);
                kq = ggml_mul_mat(ctx0, k, q_scaled_attn);
                // if (debug_siglip_layer) {
                //     cb(q_scaled, pi05_clip_debug_siglip_name(il, "q_scaled").c_str(), -1);
                //     cb(build_bf16_roundtrip(ggml_mul_mat(ctx0, k, q)), pi05_clip_debug_siglip_name(il, "attn_logits_raw").c_str(), -1);
                // }
                // if (debug_siglip_layer) {
                //     cb(kq, pi05_clip_debug_siglip_name(il, "attn_logits_scaled").c_str(), -1);
                // }
                // kq = build_bf16_roundtrip(kq);
                kq = ggml_soft_max_ext(ctx0, kq, kq_mask, 1.0f, 0.0f);
                // kq = build_bf16_roundtrip(kq);
                if (debug_siglip_layer) {
                    cb(kq, pi05_clip_debug_siglip_name(il, "attn_probs").c_str(), -1);
                }
            } else {
            ...
            ggml_tensor * kqv_bthd = ggml_permute(ctx0, kqv, 0, 2, 1, 3);
            if (debug_siglip_layer) {
                cb(kqv_bthd, pi05_clip_debug_siglip_name(il, "kqv_bthd_raw").c_str(), -1);
            }
            cur = ggml_cont_2d(ctx0, kqv_bthd, kqv_bthd->ne[0]*n_head, n_tokens);
            // if (pi0_bf16_dense) {
            //     cur = build_bf16_roundtrip(cur);
            // }
        }
        ...
        cb(cur, "kqv_out", il);
        if (debug_siglip_layer) {
            cb(cur, pi05_clip_debug_siglip_name(il, "kqv").c_str(), -1);
        }

        // if (wo) {
        //     cur = ggml_mul_mat(ctx0, pi0_bf16_dense ? pi05_clip_preserve_quantized_weight(ctx0, wo) : wo, cur);
        // }
        ...
        if (wo_b) {
            // cur = ggml_add(ctx0, cur, pi0_bf16_dense ? build_bf16_roundtrip(wo_b) : wo_b);
        ...
        // }
        if (debug_siglip_layer) {
            cb(cur, pi05_clip_debug_siglip_name(il, "attn_out_manual").c_str(), -1);
        }
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch & imgs, bool persist_pi0_cache = false) {
    ...
    if (persist_pi0_cache && ctx->proj_type() == PROJECTOR_TYPE_PI0) {
        ctx->pi0_ctx0_ptr  = std::move(graph.ctx0_ptr);
        ctx->pi0_gf        = res;
        ctx->pi0_cached_nx = imgs.entries[0]->nx;
        ctx->pi0_cached_ny = imgs.entries[0]->ny;
    }

    return res;
}

// P0: batched graph builder for PI0 ViT
static ggml_cgraph * clip_image_build_graph_batched(clip_ctx * ctx, const clip_image_f32_batch & imgs, int n_images) {
    GGML_ASSERT(!imgs.entries.empty());
    GGML_ASSERT(ctx->proj_type() == PROJECTOR_TYPE_PI0);
    clip_graph graph(ctx, *imgs.entries[0]);
    return graph.build_pi0_batched(n_images);
}
...
struct clip_model_loader {
    ...
    void load_hparams(clip_model & model, clip_modality modality) {
        ...
        // other hparams
        {
            ...
            hparams.has_llava_projector = model.proj_type == PROJECTOR_TYPE_MLP
                                       ...
                                       || model.proj_type == PROJECTOR_TYPE_PI0;
    ...
    void load_tensors(clip_ctx & ctx_clip) {
        ...
        switch (model.proj_type) {
            ...
            case PROJECTOR_TYPE_PI0:
                {
                    model.mm_w = get_tensor(string_format(TN_PI0_PROJ, "weight"));
                    model.mm_b = get_tensor(string_format(TN_PI0_PROJ, "bias"));
                } break;
            default:
...
void print_vector2(const std::vector<float>& vec, const std::string& name = "Vector") {
    GGML_UNUSED(vec);
    GGML_UNUSED(name);
}

static void normalize_image_u8_to_f32_pi0(const clip_image_u8 & src, clip_image_f32 & dst, const float mean[3], const float std[3]) {
    GGML_UNUSED(mean);
    GGML_UNUSED(std);

    dst.nx = src.nx;
    dst.ny = src.ny;
    dst.inp_raw_layout = false;
    dst.buf.resize(src.buf.size());

    // TODO @ngxson : seems like this could be done more efficiently on cgraph
    for (size_t i = 0; i < src.buf.size(); ++i) {
        dst.buf[i] = static_cast<float>(src.buf[i]) / 255.0f * 2.0f - 1.0f;
    }
}

// Planar CHW layout for graph inp_raw (see pi0_vit_optimization.md).
static void normalize_image_u8_to_f32_pi0_inp_raw(const clip_image_u8 & src, clip_image_f32 & dst, const float mean[3], const float std[3]) {
    GGML_UNUSED(mean);
    GGML_UNUSED(std);

    dst.nx = src.nx;
    dst.ny = src.ny;
    dst.inp_raw_layout = true;

    const int n = src.nx * src.ny;
    dst.buf.resize((size_t) 3 * n);

    for (int i = 0; i < n; ++i) {
        const float r = static_cast<float>(src.buf[(size_t) 3 * i    ]) / 255.0f * 2.0f - 1.0f;
        const float g = static_cast<float>(src.buf[(size_t) 3 * i + 1]) / 255.0f * 2.0f - 1.0f;
        const float b = static_cast<float>(src.buf[(size_t) 3 * i + 2]) / 255.0f * 2.0f - 1.0f;
        dst.buf[(size_t) i]       = r;
        dst.buf[(size_t) n + i]   = g;
        dst.buf[(size_t) 2 * n + i] = b;
    }
}

// OpenPI / xr-1 image_tools.resize_with_pad uses jax.image.resize(LINEAR, antialias=True):
// half-pixel centers, triangle kernel, separable 1D resize, round to uint8, zero-pad centered.
static void pi0_compute_jax_resize_weights(int input_size, int output_size, std::vector<float> & weights) {
    weights.assign((size_t) output_size * (size_t) input_size, 0.f);
    ...
    if (input_size == output_size) {
        for (int o = 0; o < output_size; ++o) {
            weights[(size_t) o * input_size + o] = 1.f;
        }
        return;
    }

    const float scale       = (float) output_size / (float) input_size;
    const float inv_scale   = 1.0f / scale;
    const float kernel_scale = std::max(inv_scale, 1.0f);

    for (int o = 0; o < output_size; ++o) {
        const float sample_f = (o + 0.5f) * inv_scale - 0.5f;
        float * row = weights.data() + (size_t) o * input_size;
        float sum = 0.f;
        for (int i = 0; i < input_size; ++i) {
            const float x = std::abs(sample_f - (float) i) / kernel_scale;
            const float w = std::max(0.f, 1.f - x);
            row[i] = w;
            sum += w;
        }
        if (std::abs(sum) > 1000.f * std::numeric_limits<float>::epsilon()) {
            for (int i = 0; i < input_size; ++i) {
                row[i] /= sum;
            }
        } else {
            std::fill(row, row + input_size, 0.f);
        }
        if (sample_f < -0.5f || sample_f > (float) input_size - 0.5f) {
            std::fill(row, row + input_size, 0.f);
        }
    }
}

static void pi0_resize_jax_linear_antialias(const clip_image_u8 & src, clip_image_u8 & dst) {
    const int dst_w = dst.nx;
    const int dst_h = dst.ny;
    ...
    std::vector<float> w_w;
    pi0_compute_jax_resize_weights(src_h, dst_h, w_h);
    pi0_compute_jax_resize_weights(src_w, dst_w, w_w);

    const size_t tmp_size = (size_t) dst_h * (size_t) src_w;
    std::vector<float> tmp(tmp_size);
    dst.buf.resize((size_t) 3 * dst_w * dst_h);

    for (int c = 0; c < 3; ++c) {
        for (int oh = 0; oh < dst_h; ++oh) {
            const float * w_row_h = w_h.data() + (size_t) oh * src_h;
            for (int iw = 0; iw < src_w; ++iw) {
                float v = 0.f;
                for (int ih = 0; ih < src_h; ++ih) {
                    const float w = w_row_h[ih];
                    if (w != 0.f) {
                        v += w * (float) src.buf[3 * ((size_t) ih * src_w + iw) + c];
                    }
                }
                tmp[(size_t) oh * src_w + iw] = v;
            }
        }

        for (int oh = 0; oh < dst_h; ++oh) {
            for (int ow = 0; ow < dst_w; ++ow) {
                const float * w_row_w = w_w.data() + (size_t) ow * src_w;
                float v = 0.f;
                for (int iw = 0; iw < src_w; ++iw) {
                    const float w = w_row_w[iw];
                    if (w != 0.f) {
                        v += w * tmp[(size_t) oh * src_w + iw];
                    }
                }
                const float clamped = std::max(0.f, std::min(v, 255.f));
                dst.buf[3 * ((size_t) oh * dst_w + ow) + c] = (uint8_t) std::lround(clamped);
            }
        }
    }
}

static void pi0_resize_with_pad_openpi(
    ...
    dst.nx = target_w;
    dst.ny = target_h;
    dst.buf.assign((size_t) 3 * target_w * target_h, 0);

    if (src.nx <= 0 || src.ny <= 0) {
        return;
    }

    if (src.nx == target_w && src.ny == target_h) {
        dst.buf = src.buf;
        return;
    }

    const float ratio = std::max(
        (float) src.nx / (float) target_w,
        (float) src.ny / (float) target_h);
    int new_w = (int) ((float) src.nx / ratio);
    int new_h = (int) ((float) src.ny / ratio);
    new_w = std::max(1, std::min(new_w, target_w));
    new_h = std::max(1, std::min(new_h, target_h));

    clip_image_u8 resized;
    resized.nx = new_w;
    resized.ny = new_h;
    pi0_resize_jax_linear_antialias(src, resized);

    const int pad_w0 = (target_w - new_w) / 2;
    const int pad_h0 = (target_h - new_h) / 2;
    for (int y = 0; y < new_h; ++y) {
        for (int x = 0; x < new_w; ++x) {
            const int dx = x + pad_w0;
            const int dy = y + pad_h0;
            const size_t dst_idx = 3 * ((size_t) dy * target_w + (size_t) dx);
            const size_t src_idx = 3 * ((size_t) y * new_w + (size_t) x);
            dst.buf[dst_idx + 0] = resized.buf[src_idx + 0];
            dst.buf[dst_idx + 1] = resized.buf[src_idx + 1];
            dst.buf[dst_idx + 2] = resized.buf[src_idx + 2];
        }
    }
}
...
bool clip_image_preprocess(struct clip_ctx * ctx, const clip_image_u8 * img, struct clip_image_f32_batch * res_imgs) {
    ...
    switch (ctx->proj_type()) {
        ...
        case PROJECTOR_TYPE_PI0:
            {
                const bool vit_perf = clip_pi0_vit_perf_enabled();
                const auto t_preprocess_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};

                if (pi_model_debug_dump_enabled()) {
                    const char * dump_path = pi_model_debug_dump_file();
                    std::ofstream ofs(dump_path, std::ios::app);
                    if (ofs.is_open()) {
                        ofs << "[clip] pi_model=" << pi_model_kind_name(pi_model_kind_from_env())
                            << " preprocess_adapter=" << (clip_pi_model_use_pi05_vit_adapter() ? "pi05_openpi_resize_with_pad" : "pi0_llava_uhd_slice")
                            << " input_size=[" << original_size.width << "," << original_size.height << "]\n";
                    }
                }

                if (clip_pi_model_use_pi05_vit_adapter()) {
                    // Match xr-1 / OpenPI ResizeImages + image_tools.resize_with_pad:
                    // aspect-preserving bilinear resize + black (0) pad to image_size x image_size.
                    const int sz = params.image_size;
                    GGML_ASSERT(sz > 0);

                    const auto t_resize_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
                    clip_image_u8 resized_image;
                    pi0_resize_with_pad_openpi(*img, resized_image, sz, sz);
                    if (vit_perf) {
                        ctx->pi0_last_vit_perf.preprocess_slice_ms = clip_perf_elapsed_ms(t_resize_begin, clip_perf_clock::now());
                    }

                    const auto t_norm_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
                    clip_image_f32_ptr res(clip_image_f32_init());
                    normalize_image_u8_to_f32_pi0_inp_raw(resized_image, *res, params.image_mean, params.image_std);
                    res_imgs->entries.push_back(std::move(res));
                    if (vit_perf) {
                        ctx->pi0_last_vit_perf.preprocess_normalize_ms = clip_perf_elapsed_ms(t_norm_begin, clip_perf_clock::now());
                        ctx->pi0_last_vit_perf.preprocess_total_ms     = clip_perf_elapsed_ms(t_preprocess_begin, clip_perf_clock::now());
                    }

                    res_imgs->grid_x = 0;
                    pi05_clip_debug_dump_host_vector(
                        "pi_model_clip_preprocess_pi05_openpi_f32",
                        res_imgs->entries.back()->buf,
                        3,
                        res_imgs->entries.back()->nx,
                        res_imgs->entries.back()->ny,
                        1);
                    res_imgs->grid_y = 0;
                } else {
                    const auto t_slice_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
                    auto const inst = llava_uhd::get_slice_instructions(ctx, original_size);
                    std::vector<clip_image_u8_ptr> imgs = llava_uhd::slice_image(img, inst);
                    if (vit_perf) {
                        ctx->pi0_last_vit_perf.preprocess_slice_ms = clip_perf_elapsed_ms(t_slice_begin, clip_perf_clock::now());
                    }

                    const auto t_norm_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
                    for (size_t i = 0; i < imgs.size(); ++i) {
                        clip_image_f32_ptr res(clip_image_f32_init());
                        normalize_image_u8_to_f32_pi0(*imgs[i], *res, params.image_mean, params.image_std);
                        res_imgs->entries.push_back(std::move(res));
                    }
                    if (vit_perf) {
                        ctx->pi0_last_vit_perf.preprocess_normalize_ms = clip_perf_elapsed_ms(t_norm_begin, clip_perf_clock::now());
                    ...
                    if (!res_imgs->entries.empty()) {
                        pi05_clip_debug_dump_host_vector(
                            "pi_model_clip_preprocess_pi0_slice0_f32",
                            res_imgs->entries.front()->buf,
                            3,
                            res_imgs->entries.front()->nx,
                            res_imgs->entries.front()->ny,
                            (int64_t) res_imgs->entries.size());
                    }
                    res_imgs->grid_x = inst.grid_size.width;
                    res_imgs->grid_y = inst.grid_size.height;
                    if (pi_model_debug_dump_enabled()) {
                        ...
                        std::ofstream ofs(dump_path, std::ios::app);
                        if (ofs.is_open()) {
                            ofs << "[clip] pi0_slice_count=" << res_imgs->entries.size()
                                << " grid=[" << res_imgs->grid_x << "," << res_imgs->grid_y << "]\n";
                        }
                    }
                }
            } break;
...
static bool clip_image_batch_encode_pi0(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch & imgs, float * vec) {
    const bool vit_perf = clip_pi0_vit_perf_enabled();
    const auto t_encode_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
    ...
    if (!clip_pi0_graph_reuse_disabled() &&
        ctx->pi0_gf != nullptr &&
        ctx->pi0_cached_nx == image_size_width &&
        ctx->pi0_cached_ny == image_size_height) {
        gf           = ctx->pi0_gf;
        graph_reused = true;
    } else {
        clip_pi0_invalidate_graph_cache(ctx);
    }

    double graph_build_alloc_ms = 0.0;
    if (!graph_reused) {
        ctx->debug_print_tensors.clear();
        ggml_backend_sched_reset(ctx->sched.get());

        const auto t_build_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
        gf = clip_image_build_graph(ctx, imgs, true);
        ggml_backend_sched_alloc_graph(ctx->sched.get(), gf);
        if (vit_perf) {
            graph_build_alloc_ms = clip_perf_elapsed_ms(t_build_begin, clip_perf_clock::now());
        }

        const int n_pos_tokens = pos_w * pos_h;
        ctx->pi0_positions_buf.resize(n_pos_tokens);
        for (int i = 0; i < n_pos_tokens; ++i) {
            ctx->pi0_positions_buf[i] = i;
        }
    }

    const auto t_set_inputs_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
    ...
    auto set_input_f32 = [&get_inp_tensor](const char * name, const void * data, size_t nbytes) {
        ggml_tensor * cur = get_inp_tensor(name);
        GGML_ASSERT(cur->type == GGML_TYPE_F32);
        GGML_ASSERT(ggml_nbytes(cur) == nbytes);
        ggml_backend_tensor_set(cur, data, 0, nbytes);
    };
    ...
    const clip_image_f32 & img0 = *imgs.entries[0];
    if (img0.inp_raw_layout) {
        set_input_f32("inp_raw", img0.buf.data(), ggml_nbytes(get_inp_tensor("inp_raw")));
    } else {
        const int nx = img0.nx;
        const int ny = img0.ny;
        const int n  = nx * ny;
        std::vector<float> inp_raw((size_t) 3 * n);
        for (int y = 0; y < ny; y++) {
            for (int x = 0; x < nx; x++) {
                const size_t base_src = 3 * (size_t) (y * nx + x);
                const size_t base_dst = (size_t) (y * nx + x);
                inp_raw[base_dst]                  = img0.buf[base_src];
                inp_raw[(size_t) n + base_dst]     = img0.buf[base_src + 1];
                inp_raw[(size_t) 2 * n + base_dst] = img0.buf[base_src + 2];
            }
        }
        set_input_f32("inp_raw", inp_raw.data(), inp_raw.size() * sizeof(float));
    }

    pi05_clip_debug_dump_host_vector(
        "pi05_dbg_siglip_input",
        imgs.entries[0]->buf,
        3,
        image_size_width,
        image_size_height,
    ...
    if (!graph_reused && !ctx->pi0_positions_buf.empty()) {
        set_input_i32("positions_data", ctx->pi0_positions_buf);
    }

    const double set_inputs_ms = vit_perf ? clip_perf_elapsed_ms(t_set_inputs_begin, clip_perf_clock::now()) : 0.0;
    ...
    const auto t_compute_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
    const auto status = ggml_backend_sched_graph_compute(ctx->sched.get(), gf);
    const double graph_compute_ms = vit_perf ? clip_perf_elapsed_ms(t_compute_begin, clip_perf_clock::now()) : 0.0;
    ...
    pi05_clip_debug_dump_graph_tensors_to_file(ctx->sched.get(), gf);

    ggml_tensor * embeddings = ggml_graph_node(gf, -1);
    const int expected_n_tokens_out = clip_n_output_tokens(ctx, imgs.entries[0].get());
    if (embeddings->ne[1] != expected_n_tokens_out) {
        LOG_ERR("%s: expected output %d tokens, got %d\n", __func__, expected_n_tokens_out, (int) embeddings->ne[1]);
        GGML_ABORT("Invalid number of output tokens");
    }

    const auto t_output_get_begin = vit_perf ? clip_perf_clock::now() : clip_perf_clock::time_point{};
    ggml_backend_tensor_get(embeddings, vec, 0, ggml_nbytes(embeddings));
    const double output_get_ms = vit_perf ? clip_perf_elapsed_ms(t_output_get_begin, clip_perf_clock::now()) : 0.0;

    if (vit_perf) {
        const double encode_total_ms = clip_perf_elapsed_ms(t_encode_begin, clip_perf_clock::now());
        ctx->pi0_last_vit_perf.graph_build_alloc_ms = graph_build_alloc_ms;
        ctx->pi0_last_vit_perf.set_inputs_ms        = set_inputs_ms;
        ctx->pi0_last_vit_perf.graph_compute_ms     = graph_compute_ms;
        ctx->pi0_last_vit_perf.output_get_ms        = output_get_ms;
        ctx->pi0_last_vit_perf.encode_total_ms      = encode_total_ms;
        ctx->pi0_last_vit_perf.graph_reused         = graph_reused;

        fprintf(stderr,
            "========== PI0 VIT encode latency ==========\n"
            "  preprocess slice:        %.2f ms\n"
            "  preprocess normalize:    %.2f ms\n"
            "  preprocess total:        %.2f ms\n"
            "  graph_build+alloc:       %.2f ms (reused=%d)\n"
            "  set_inputs:              %.2f ms\n"
            "  graph_compute:           %.2f ms\n"
            "  output_get:              %.2f ms\n"
            "  encode total:            %.2f ms\n",
            ctx->pi0_last_vit_perf.preprocess_slice_ms,
            ctx->pi0_last_vit_perf.preprocess_normalize_ms,
            ctx->pi0_last_vit_perf.preprocess_total_ms,
            graph_build_alloc_ms,
            graph_reused ? 1 : 0,
            set_inputs_ms,
            graph_compute_ms,
            output_get_ms,
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs_c_ptr, float * vec) {
    ...
    // P0: For PI0, support true batching (batch_size > 1)
    const bool pi0_use_pi05_vit_adapter = ctx->model.proj_type == PROJECTOR_TYPE_PI0 && clip_pi_model_use_pi05_vit_adapter();
    if (batch_size != 1 && !pi0_use_pi05_vit_adapter) {
        return false;
    }

    // P0: batched ViT encoding for PI0
    if (batch_size > 1 && pi0_use_pi05_vit_adapter) {
        ctx->debug_print_tensors.clear();
        ggml_backend_sched_reset(ctx->sched.get());
        ggml_cgraph * gf = clip_image_build_graph_batched(ctx, imgs, batch_size);
        ggml_backend_sched_alloc_graph(ctx->sched.get(), gf);
        ...
        const int image_size_width  = imgs.entries[0]->nx;
        const int image_size_height = imgs.entries[0]->ny;
        const int patch_size_local  = hparams.patch_size;
        const int num_patches_local = (image_size_width / patch_size_local) * (image_size_height / patch_size_local);

        {
            const int nx = image_size_width;
            const int ny = image_size_height;
            const int n = nx * ny;
            std::vector<float> inp_raw(n * 3 * batch_size);

            for (int b = 0; b < batch_size; b++) {
                const auto & img_b = *imgs.entries[b];
                if (img_b.inp_raw_layout) {
                    std::memcpy(inp_raw.data() + b * (3 * n),
                                img_b.buf.data(), 3 * n * sizeof(float));
                } else {
                    ...
                    for (int y = 0; y < ny; y++) {
                        for (int x = 0; x < nx; x++) {
                            ...
                            batch_entry[        base_dst] = img_b.buf[base_src    ];
                            batch_entry[1*n +   base_dst] = img_b.buf[base_src + 1];
                            batch_entry[2*n +   base_dst] = img_b.buf[base_src + 2];
                        }
                    }
                }
            }
            ...
            ggml_backend_tensor_set(inp, inp_raw.data(), 0, inp_raw.size() * sizeof(float));
        }

        {
            std::vector<int32_t> positions_data(num_patches_local);
            for (int i = 0; i < num_patches_local; i++) {
                positions_data[i] = i;
            }
            ggml_tensor * pos_tensor = ggml_graph_get_tensor(gf, "positions_data");
            if (pos_tensor) {
                ggml_backend_tensor_set(pos_tensor, positions_data.data(), 0, positions_data.size() * sizeof(int32_t));
            }
        }
        ...
        if (status != GGML_STATUS_SUCCESS) {
            LOG_ERR("%s: batched ViT compute failed with error %d\n", __func__, status);
            return false;
        }

        pi05_clip_print_vit_debug_tensors(ctx->sched.get(), gf);
        ggml_tensor * result = ggml_graph_node(gf, ggml_graph_n_nodes(gf) - 1);
        if (pi_model_debug_dump_enabled()) {
            const char * dump_path = pi_model_debug_dump_file();
            std::ofstream ofs(dump_path, std::ios::app);
            if (ofs.is_open()) {
                ofs << "[clip] name=pi_model_clip_batched_vit_out shape=["
                    << result->ne[0] << "," << result->ne[1] << "," << result->ne[2] << "," << result->ne[3]
                    << "] batch_size=" << batch_size << "\n";
            }
        }
        int n_mmproj_embd = result->ne[0];
        int n_patches_out = result->ne[1];

        size_t per_image_bytes = n_mmproj_embd * n_patches_out * sizeof(float);
        for (int b = 0; b < batch_size; b++) {
            ggml_backend_tensor_get(result, vec + b * n_mmproj_embd * n_patches_out,
                b * per_image_bytes, per_image_bytes);
        }

        return true;
    }

    if (pi0_use_pi05_vit_adapter) {
        return clip_image_batch_encode_pi0(ctx, n_threads, imgs, vec);
    }
    ...
    if (!imgs.is_audio) {
        ...
        if (model.proj_type == PROJECTOR_TYPE_PI0) {
            pi05_clip_debug_dump_host_vector(
                "pi05_dbg_siglip_input",
                imgs.entries[0]->buf,
                3,
        ...
        if (model.proj_type == PROJECTOR_TYPE_PI0){
            const int n_pos_tokens = pos_w * pos_h;
            const int n_dim = 4304;
            std::vector<int32_t> positions_data(n_pos_tokens);
            for (int i = 0; i < n_pos_tokens; i++) {
                    positions_data[i] = i;
                }
            set_input_i32("positions_data", positions_data);

            // std::vector<float> ones(n_dim*n_pos_tokens);
            // for (int i = 0; i < n_pos_tokens; i++) {
            //         ones[i] = 1.0f;
            //     }
            // set_input_f32("ones", ones);
        }
    ...
    if (ctx->model.proj_type == PROJECTOR_TYPE_PI0) {
        pi05_clip_debug_dump_graph_tensors_to_file(ctx->sched.get(), gf);
    }
    ...
    ggml_backend_tensor_get(embeddings, vec, 0, ggml_nbytes(embeddings));
    // print_vector2(std::vector<float>(vec, vec + 1152), "vit_output");
    // printf("\n\n\n");
    // print_vector2(std::vector<float>(vec + 1152, vec + 2*1152), "vit_output");
    // printf("\n\n\n");
    // print_vector2(std::vector<float>(vec + 2*1152, vec + 3*1152), "vit_output");
...
bool mtmd_is_pi0_model_inner(const struct clip_ctx * ctx){
    return ctx->proj_type() == PROJECTOR_TYPE_PI0;
}

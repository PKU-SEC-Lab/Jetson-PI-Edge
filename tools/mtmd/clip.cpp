...
static bool clip_use_pi0_legacy_attention(const clip_model & model) {
    return model.proj_type == PROJECTOR_TYPE_PI0 &&
...
ggml_tensor * clip_graph::build_attn(
    ...
    const bool use_pi0_legacy_attention = clip_use_pi0_legacy_attention(model);
    ggml_tensor * q = ggml_permute(ctx0, q_cur, 0, 2, 1, 3);
    if (use_pi0_legacy_attention && q->type != GGML_TYPE_F32) {
    ...
    // Legacy PI0 has n_embd=1152 and n_head=16. Keeping head_dim=72 also
    // pins the CUDA FLASH_ATTN_EXT dispatcher to its generic tile kernel.
    GGML_ASSERT(!use_pi0_legacy_attention || q->ne[0] == 72);
    ...
    if (flash_attn_type == CLIP_FLASH_ATTN_TYPE_ENABLED) {
        ...
        GGML_ASSERT(!use_pi0_legacy_attention || q->type == GGML_TYPE_F32);
        GGML_ASSERT(!use_pi0_legacy_attention || k->type == GGML_TYPE_F16);
        GGML_ASSERT(!use_pi0_legacy_attention || v->type == GGML_TYPE_F16);
...
struct clip_model_loader {
    ...
    void load_tensors(clip_ctx & ctx_clip) {
        ...
        switch (model.proj_type) {
            ...
            case PROJECTOR_TYPE_PI0:
                {
                    model.mm_fc_w = get_tensor(string_format(TN_PI0_PROJ, "weight"));
                    model.mm_fc_b = get_tensor(string_format(TN_PI0_PROJ, "bias"));
                } break;
            case PROJECTOR_TYPE_LFM2:
                {
...
struct clip_init_result clip_init(const char * fname, struct clip_context_params ctx_params) {
    ...
    try {
        ...
        if (loader.has_vision) {
            ...
            if (clip_use_pi0_legacy_attention(ctx_vision->model)) {
                ctx_vision->flash_attn_type = CLIP_FLASH_ATTN_TYPE_ENABLED;
                LOG_INF("%s: PI0 legacy ViT attention locked: Q=f32, K/V=f16, "
                        "accumulation=f32, flash=enabled, CUDA head_dim=72 tile kernel\n",
                        __func__);
            }

...
MTMD_API mtmd_context * mtmd_init_from_file_with_device(
...
MTMD_API bool mtmd_is_pi0_model(mtmd_context * ctx);

MTMD_API void mtmd_pi0_clear_encode_cache(mtmd_context * ctx);
MTMD_API bool mtmd_pi0_apply_cached_chunk_embd(mtmd_context * ctx, size_t chunk_index);
MTMD_API int32_t mtmd_pi0_preencode_image_chunks(mtmd_context * ctx, const mtmd_input_chunks * chunks);
...
// image_indices: array of chunk indices that are images
// returns 0 on success, output embeddings contain all images concatenated
MTMD_API int32_t mtmd_encode_chunks_batched(mtmd_context * ctx,

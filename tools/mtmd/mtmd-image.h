...
// Legacy PI0 path: preserve LLaVA-UHD pixels and overview-first view ordering.
struct mtmd_image_preprocessor_pi0_legacy : mtmd_image_preprocessor_llava_uhd {
    mtmd_image_preprocessor_pi0_legacy(const clip_ctx * ctx) : mtmd_image_preprocessor_llava_uhd(ctx) {}
    mtmd_image_preproc_out preprocess(const clip_image_u8 & img) override;
};

// PI0.5 / OpenPI path: aspect-preserving JAX linear-antialias resize followed
// by centered black padding to the vision encoder's fixed input size.
struct mtmd_image_preprocessor_pi05_openpi : mtmd_image_preprocessor {
    mtmd_image_preprocessor_pi05_openpi(const clip_ctx * ctx) : mtmd_image_preprocessor(ctx) {}
    mtmd_image_preproc_out preprocess(const clip_image_u8 & img) override;
};

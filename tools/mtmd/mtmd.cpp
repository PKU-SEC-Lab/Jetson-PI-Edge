...
struct mtmd_context {
    ...
    // PI0 legacy: chunk_index -> embedding, filled before eval.
    std::unordered_map<size_t, std::vector<float>> pi0_chunk_embd_cache;
...
mtmd_context * mtmd_init_from_file(const char * mmproj_fname,
    ...
    return mtmd_init_from_file_with_device(mmproj_fname, text_model, ctx_params, nullptr);
}

mtmd_context * mtmd_init_from_file_with_device(const char * mmproj_fname,
        const struct llama_model * text_model,
        const struct mtmd_context_params ctx_params,
        ggml_backend_dev_t device) {
    try {
        return new mtmd_context(mmproj_fname, text_model, ctx_params, device);
...
bool mtmd_is_pi0_model(mtmd_context * ctx){
    return mtmd_is_pi0_model_inner(ctx->ctx_v);
}

void mtmd_pi0_clear_encode_cache(mtmd_context * ctx) {
    if (ctx != nullptr) {
        ctx->pi0_chunk_embd_cache.clear();
    }
}

bool mtmd_pi0_apply_cached_chunk_embd(mtmd_context * ctx, size_t chunk_index) {
    if (ctx == nullptr) {
        return false;
    }
    const auto it = ctx->pi0_chunk_embd_cache.find(chunk_index);
    if (it == ctx->pi0_chunk_embd_cache.end()) {
        return false;
    }
    ctx->image_embd_v = it->second;
    return true;
}

int32_t mtmd_pi0_preencode_image_chunks(mtmd_context * ctx, const mtmd_input_chunks * chunks) {
    if (ctx == nullptr || chunks == nullptr || !mtmd_is_pi0_model(ctx)) {
        return 0;
    }

    static int disabled = -1;
    if (disabled < 0) {
        const char * env = std::getenv("CLIP_PI0_VIT_PREENCODE_DISABLE");
        disabled = (env != nullptr && env[0] != '\0' && env[0] != '0') ? 1 : 0;
    }
    if (disabled) {
        return 0;
    }
    ...
    for (size_t i = 0; i < n_chunks; ++i) {
        ...
        ctx->pi0_chunk_embd_cache[i] = ctx->image_embd_v;
        ++n_images;
    }

    if (n_images > 0) {
        LOG_INF("%s: pre-encoded %d image chunk(s) in %.2f ms (VIT before LLM; clip_ctx=%p keeps pi0_gf across requests)\n",
            __func__, n_images, (ggml_time_us() - t0) / 1000.0, (void *) ctx->ctx_v);
    }
...
struct mtmd_tokenizer {
    ...
    int32_t tokenize(mtmd_input_chunks * output) {
        ...
        // mtmd_input_chunk combined_chunk;
        // combined_chunk.type = MTMD_INPUT_CHUNK_TYPE_TEXT; // Start with text type
...
void mtmd_pi0_result_apply_vit_perf(mtmd_context * ctx, mtmd_pi0_result * result) {
    if (ctx == nullptr || result == nullptr || ctx->ctx_v == nullptr) {
        return;
    }

    const clip_pi0_vit_perf * vp = clip_get_last_pi0_vit_perf(ctx->ctx_v);
    if (vp == nullptr) {
        return;
    }

    result->vit_preprocess_ms      = vp->preprocess_total_ms;
    result->vit_graph_build_alloc_ms = vp->graph_build_alloc_ms;
    result->vit_set_inputs_ms        = vp->set_inputs_ms;
    result->vit_graph_compute_ms     = vp->graph_compute_ms;
    result->vit_output_get_ms        = vp->output_get_ms;
    result->vit_graph_reused         = vp->graph_reused;
    result->has_vit_breakdown        = true;
}
...
// P0: Batch-encode multiple image chunks in a single ViT forward pass
int32_t mtmd_encode_chunks_batched(mtmd_context * ctx,
                                   const mtmd_input_chunks * chunks,
                                   const size_t * image_indices,
                                   size_t n_image_indices) {
    if (n_image_indices == 0) return 0;
    if (!ctx->ctx_v) {
        LOG_ERR("%s: model does not support vision input\n", __func__);
        return 1;
    }
    ...
    clip_ctx * ctx_clip = ctx->ctx_v;
    int n_mmproj_embd = clip_n_mmproj_embd(ctx_clip);
    int n_images = (int)n_image_indices;

    // Collect all image data into a single batch
    clip_image_f32_batch batch;
    batch.is_audio = false;

    for (size_t ii = 0; ii < n_image_indices; ii++) {
        size_t idx = image_indices[ii];
        auto chunk = mtmd_input_chunks_get(chunks, idx);
        if (mtmd_input_chunk_get_type(chunk) != MTMD_INPUT_CHUNK_TYPE_IMAGE) {
            LOG_ERR("%s: chunk %zu is not an image\n", __func__, idx);
            return 1;
        }
        const auto * img_tokens = chunk->tokens_image.get();
        if (!img_tokens || img_tokens->batch_f32.entries.empty()) {
            LOG_ERR("%s: chunk %zu has no image data\n", __func__, idx);
            return 1;
        }
        for (const auto & entry : img_tokens->batch_f32.entries) {
            batch.entries.emplace_back(new clip_image_f32(*entry));
        }
    }

    // Calculate total output size
    int n_tokens_per_image = clip_n_output_tokens(ctx_clip, batch.entries[0].get());
    int total_tokens = n_tokens_per_image * n_images;
    ctx->image_embd_v.resize(total_tokens * n_mmproj_embd);

    // Call batched encode
    bool ok = clip_image_batch_encode(ctx_clip, ctx->n_threads, &batch, ctx->image_embd_v.data());
    return ok ? 0 : 1;
}

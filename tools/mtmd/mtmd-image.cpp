...
static bool mtmd_image_use_pi0_normalize() {
    return pi_model_kind_is_pi0(pi_model_kind_from_env());
}

static void mtmd_image_u8_to_pi0_f32(const clip_image_u8 & src, clip_image_f32 & dst) {
    if (src.is_placeholder()) {
        dst.set_size(src.get_size(), true, false);
        return;
    }

    const auto & src_buf = src.get_ro_buf();
    std::vector<float> dst_buf(src_buf.size());
    for (size_t i = 0; i < src_buf.size(); ++i) {
        dst_buf[i] = (static_cast<float>(src_buf[i]) / 255.0f) * 2.0f - 1.0f;
    }
    dst.set_size(src.get_size(), false, false);
    dst.cpy_buf(dst_buf);
}

// Match jax.image.resize(..., method="linear", antialias=true): half-pixel
// sample centers, a scaled triangle kernel when downsampling, and normalized
// separable weights.
static void pi0_compute_jax_resize_weights(
        int input_size,
        int output_size,
        std::vector<float> & weights) {
    weights.assign((size_t) output_size * (size_t) input_size, 0.0f);
    ...
    if (input_size == output_size) {
        for (int o = 0; o < output_size; ++o) {
            weights[(size_t) o * input_size + o] = 1.0f;
        }
        return;
    }
    ...
    for (int o = 0; o < output_size; ++o) {
        ...
        for (int i = 0; i < input_size; ++i) {
            ...
            const float w = std::max(0.0f, 1.0f - x);
            row[i] = w;
            sum += w;
        }
        if (std::abs(sum) > 1000.0f * std::numeric_limits<float>::epsilon()) {
            for (int i = 0; i < input_size; ++i) {
                row[i] /= sum;
            }
        } else {
            std::fill(row, row + input_size, 0.0f);
        }
...
static void pi0_resize_jax_linear_antialias(
        const clip_image_u8 & src,
        clip_image_u8 & dst,
        int dst_w,
        int dst_h) {
    const clip_image_size src_size = src.get_size();
    const int src_w = src_size.width;
    const int src_h = src_size.height;
    dst.set_size({dst_w, dst_h}, src.is_placeholder());

    if (src.is_placeholder() || dst_w <= 0 || dst_h <= 0 || src_w <= 0 || src_h <= 0) {
        return;
    }
    ...
    if (dst_w == 1 && dst_h == 1) {
        dst.cpy_buf({src_buf[0], src_buf[1], src_buf[2]});
        return;
    }
    ...
    std::vector<uint8_t> dst_buf((size_t) 3 * dst_w * dst_h);
    ...
    for (int c = 0; c < 3; ++c) {
        for (int oh = 0; oh < dst_h; ++oh) {
            ...
            for (int iw = 0; iw < src_w; ++iw) {
                ...
                for (int ih = 0; ih < src_h; ++ih) {
                    ...
                    if (w != 0.0f) {
                        v += w * (float) src_buf[3 * ((size_t) ih * src_w + iw) + c];
                    }
                }
        ...
        for (int oh = 0; oh < dst_h; ++oh) {
            for (int ow = 0; ow < dst_w; ++ow) {
                ...
                const float clamped = std::max(0.0f, std::min(v, 255.0f));
                dst_buf[3 * ((size_t) oh * dst_w + ow) + c] = (uint8_t) std::lround(clamped);
            }
        }
    }
...
static void pi0_resize_with_pad_openpi(
        ...
        int target_h,
        int target_w) {
    dst.set_size({target_w, target_h}, src.is_placeholder());
    if (src.is_placeholder()) {
        return;
    }

    const clip_image_size src_size = src.get_size();
    if (src_size.width == target_w && src_size.height == target_h) {
        dst.cpy_buf(src.get_ro_buf());
        return;
    }

    const float ratio = std::max(
        (float) src_size.width  / (float) target_w,
        (float) src_size.height / (float) target_h);
    int new_w = (int) ((float) src_size.width  / ratio);
    int new_h = (int) ((float) src_size.height / ratio);
    new_w = std::max(1, std::min(new_w, target_w));
    new_h = std::max(1, std::min(new_h, target_h));

    clip_image_u8 resized;
    pi0_resize_jax_linear_antialias(src, resized, new_w, new_h);

    const int pad_w0 = (target_w - new_w) / 2;
    const int pad_h0 = (target_h - new_h) / 2;
    std::vector<uint8_t> dst_buf((size_t) 3 * target_w * target_h, 0);
    const auto & resized_buf = resized.get_ro_buf();
    for (int y = 0; y < new_h; ++y) {
        for (int x = 0; x < new_w; ++x) {
            const size_t dst_idx = 3 * ((size_t) (y + pad_h0) * target_w + (size_t) (x + pad_w0));
            const size_t src_idx = 3 * ((size_t) y * new_w + (size_t) x);
            dst_buf[dst_idx + 0] = resized_buf[src_idx + 0];
            dst_buf[dst_idx + 1] = resized_buf[src_idx + 1];
            dst_buf[dst_idx + 2] = resized_buf[src_idx + 2];
        }
    }
    dst.cpy_buf(dst_buf);
}
...
mtmd_image_preproc_out mtmd_image_preprocessor_llava_uhd::preprocess(const clip_image_u8 & img) {
    ...
    if (mtmd_image_use_pi0_normalize()) {
        mtmd_image_u8_to_pi0_f32(sliced.overview, output.overview);
        for (const auto & slice : sliced.slices) {
            clip_image_f32 dst;
            mtmd_image_u8_to_pi0_f32(slice, dst);
            output.entries.push_back(std::move(dst));
        }
    } else {
...
//
// PI image preprocessors
//

mtmd_image_preproc_out mtmd_image_preprocessor_pi0_legacy::preprocess(const clip_image_u8 & img) {
    const auto inst = get_slice_instructions(img.get_size());
    mtmd_image_preproc_out output;

    // The legacy PI0 path uses the old LLaVA-UHD resize policy. New mtmd keeps
    // the overview separate so every view can be encoded as a supported batch-1
    // chunk, while mtmd.cpp preserves the old overview-first concatenation.
    clip_image_u8 overview;
    img_tool::resize(img, overview, inst.overview_size,
                     RESIZE_ALGO_BILINEAR, PAD_CEIL, {0, 0, 0});
    mtmd_image_u8_to_pi0_f32(overview, output.overview);
    output.grid_x = inst.grid_size.width;
    output.grid_y = inst.grid_size.height;
    ...
    if (!hparams.image_res_candidates.empty()) {
        // LLaVA-1.6/pinpoint branch in the legacy implementation.
        img_tool::resize(img, refined, inst.refined_size,
                         RESIZE_ALGO_BILINEAR, PAD_CEIL, {0, 0, 0});
    } else {
        // Dynamic LLaVA-UHD branch used by the original PI0 implementation.
        img_tool::resize(img, refined, inst.refined_size,
                         RESIZE_ALGO_BICUBIC, PAD_NONE);
    }

    for (const auto & slice : inst.slices) {
        clip_image_u8 image_slice;
        img_tool::crop(refined, image_slice,
                       slice.x, slice.y, slice.size.width, slice.size.height);
        clip_image_f32 slice_f32;
        mtmd_image_u8_to_pi0_f32(image_slice, slice_f32);
        output.entries.push_back(std::move(slice_f32));
    }

    return output;
}

mtmd_image_preproc_out mtmd_image_preprocessor_pi05_openpi::preprocess(const clip_image_u8 & img) {
    const int sz = hparams.image_size;
    GGML_ASSERT(sz > 0);

    clip_image_u8 resized;
    pi0_resize_with_pad_openpi(img, resized, sz, sz);

    clip_image_f32 image_f32;
    mtmd_image_u8_to_pi0_f32(resized, image_f32);

    mtmd_image_preproc_out output;
    output.entries.push_back(std::move(image_f32));
    return output;
}

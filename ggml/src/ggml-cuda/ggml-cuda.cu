...
#    define GGML_CUDA_LOCAL_NVTX_AVAILABLE 1
#  elif __has_include(<nvToolsExt.h>)
#    include <nvToolsExt.h>
#    define GGML_CUDA_LOCAL_NVTX_AVAILABLE 1
#  else
#    define GGML_CUDA_LOCAL_NVTX_AVAILABLE 0
#  endif
#else
#  define GGML_CUDA_LOCAL_NVTX_AVAILABLE 0
#endif

#if GGML_CUDA_LOCAL_NVTX_AVAILABLE
struct ggml_cuda_local_nvtx_range {
    explicit ggml_cuda_local_nvtx_range(const char * name) {
        nvtxRangePushA(name);
    }

    ~ggml_cuda_local_nvtx_range() {
        nvtxRangePop();
    }

    ggml_cuda_local_nvtx_range(const ggml_cuda_local_nvtx_range &) = delete;
    ggml_cuda_local_nvtx_range & operator=(const ggml_cuda_local_nvtx_range &) = delete;
};

#define GGML_CUDA_NVTX_CONCAT_IMPL(x, y) x##y
#define GGML_CUDA_NVTX_CONCAT(x, y) GGML_CUDA_NVTX_CONCAT_IMPL(x, y)
#define GGML_CUDA_NVTX_SCOPE(name) \
    ggml_cuda_local_nvtx_range GGML_CUDA_NVTX_CONCAT(ggml_cuda_local_nvtx_range_, __LINE__) (name)
#else
#define GGML_CUDA_NVTX_SCOPE(name) \
    do { \
        (void) sizeof(name); \
...
static void * ggml_cuda_host_malloc(size_t size) {
    GGML_CUDA_NVTX_SCOPE("ggml/cuda/host_malloc");
    ...
    void * ptr = nullptr;
    {
        GGML_CUDA_NVTX_SCOPE("ggml/cuda/cudaMallocHost");
...
static ggml_backend_buffer_t ggml_backend_cuda_host_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    GGML_CUDA_NVTX_SCOPE("ggml/cuda/host_buffer_type_alloc_buffer");
...
static enum ggml_status ggml_backend_cuda_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    GGML_CUDA_NVTX_SCOPE("ggml/cuda/backend_graph_compute");
    ggml_backend_cuda_context * cuda_ctx = (ggml_backend_cuda_context *)backend->context;

    {
        GGML_CUDA_NVTX_SCOPE("ggml/cuda/set_device");
    ...
    {
        GGML_CUDA_NVTX_SCOPE("ggml/cuda/init_cuda_graph_obj");
    ...
    {
        GGML_CUDA_NVTX_SCOPE("ggml/cuda/check_gpu_arch_graph_support");
        if (cuda_ctx->cuda_graph->graph == nullptr) {
    ...
    {
        GGML_CUDA_NVTX_SCOPE("ggml/cuda/check_graph_disable_flags");
    ...
    if (use_cuda_graph) {
        {
            GGML_CUDA_NVTX_SCOPE("ggml/cuda/check_graph_update_required");
            cuda_graph_update_required = is_cuda_graph_update_required(cuda_ctx, cgraph);
        }

        {
            GGML_CUDA_NVTX_SCOPE("ggml/cuda/check_node_graph_compatibility");
            use_cuda_graph = check_node_graph_compatibility(cgraph, use_cuda_graph);
        }

        {
            GGML_CUDA_NVTX_SCOPE("ggml/cuda/update_consecutive_update_state");
            if (use_cuda_graph && cuda_graph_update_required) {
    ...
    if (use_cuda_graph && cuda_graph_update_required) {
        {
            GGML_CUDA_NVTX_SCOPE("ggml/cuda/capture_lock_counter");
            std::lock_guard<std::mutex> lock(ggml_cuda_lock);
            ggml_cuda_lock_counter.fetch_add(1, std::memory_order_relaxed);
        }

        {
            GGML_CUDA_NVTX_SCOPE("ggml/cuda/stream_begin_capture");
    ...
    {
        GGML_CUDA_NVTX_SCOPE("ggml/cuda/evaluate_and_capture_cuda_graph");

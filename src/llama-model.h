...
struct llama_model {
    ...
    // pi0 / pi05 action expert tensors
    struct ggml_tensor * action_in   = nullptr;
    struct ggml_tensor * action_in_b = nullptr;
    struct ggml_tensor * action_out  = nullptr;
    struct ggml_tensor * action_out_b = nullptr;
    struct ggml_tensor * action_time_in = nullptr;
    struct ggml_tensor * action_time_in_b = nullptr;
    struct ggml_tensor * action_time_out = nullptr;
    struct ggml_tensor * action_time_out_b = nullptr;
    struct ggml_tensor * state_proj = nullptr;
    struct ggml_tensor * state_proj_b = nullptr;
    struct ggml_tensor * time_mlp_in = nullptr;
    struct ggml_tensor * time_mlp_in_b = nullptr;
    struct ggml_tensor * time_mlp_out = nullptr;
    struct ggml_tensor * time_mlp_out_b = nullptr;
    struct ggml_tensor * ae_output_norm = nullptr;
    struct ggml_tensor * ae_output_norm_dense = nullptr;
    struct ggml_tensor * ae_output_norm_dense_b = nullptr;
...
#define LLAMA_LOAD_LOCALS \
    ...
    const int64_t n_embd_ae      = hparams.n_embd_ae;        GGML_UNUSED(n_embd_ae); \
    const int64_t n_ff_ae        = hparams.n_ff_ae();        GGML_UNUSED(n_ff_ae); \
    const int64_t n_action_dim   = hparams.action_dim;       GGML_UNUSED(n_action_dim); \

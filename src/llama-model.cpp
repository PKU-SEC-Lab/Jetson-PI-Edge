...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        const int64_t n_ff_ae       = hparams.n_ff_ae();
            ...
            case LLM_ARCH_GEMMA:
                {
                    ...
                    // output
                    output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, 0);
                    output      = hparams.use_par_res ? nullptr : create_tensor(tn(LLM_TENSOR_TOKEN_EMBD,  "weight"), {n_embd, n_vocab}, TENSOR_DUPLICATED); // same as tok_embd, duplicated to allow offloading
            ...
            case LLM_ARCH_PI0:
                {
                    ...
                    // Legacy pi0 uses action_time_* and state_proj, while pi05_x2w
                    // replaces them with time_mlp_* + adaptive RMSNorm modulation.
                    // Keep both families optional at load time and let the forward path
                    // branch on tensor availability.
                    ...
                    // output
                    // Legacy pi0 stores output_norm.weight as the AE output norm
                    // ({n_embd_ae}); pi05_x2w replaces it with adaptive modulation.
                    ...
                    for (int i = n_layer/2; i < n_layer; ++i) {
                        auto & ae_layer = layers[i];

                        ae_layer.attn_norm = create_tensor(tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd_ae}, pi0_tensor_flags);
                        ae_layer.attn_norm_dense = create_tensor(tn(LLM_TENSOR_ATTN_NORM_DENSE, "weight", i), {n_embd_ae, 3 * n_embd_ae}, pi05_tensor_flags);
                        ae_layer.attn_norm_dense_b = create_tensor(tn(LLM_TENSOR_ATTN_NORM_DENSE, "bias", i), {3 * n_embd_ae}, pi05_tensor_flags);

                        ae_layer.wq = create_tensor(tn(LLM_TENSOR_ATTN_Q,   "weight", i), {n_embd_ae, n_embd_head_k * n_head}, 0);
                        ae_layer.wk = create_tensor(tn(LLM_TENSOR_ATTN_K,   "weight", i), {n_embd_ae, n_embd_k_gqa}, 0);
                        ae_layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V,   "weight", i), {n_embd_ae, n_embd_v_gqa}, 0);
                        ae_layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i), {n_embd_head_k * n_head, n_embd_ae}, 0);

                        ae_layer.ffn_norm = create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd_ae}, pi0_tensor_flags);
                        ae_layer.ffn_norm_dense = create_tensor(tn(LLM_TENSOR_FFN_NORM_DENSE, "weight", i), {n_embd_ae, 3 * n_embd_ae}, pi05_tensor_flags);
                        ae_layer.ffn_norm_dense_b = create_tensor(tn(LLM_TENSOR_FFN_NORM_DENSE, "bias", i), {3 * n_embd_ae}, pi05_tensor_flags);
                        ae_layer.ffn_gate = create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd_ae,   n_ff_ae}, 0);
                        ae_layer.ffn_up   = create_tensor(tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd_ae,   n_ff_ae}, 0);
                        ae_layer.ffn_down = create_tensor(tn(LLM_TENSOR_FFN_DOWN, "weight", i), {  n_ff_ae, n_embd_ae}, 0);
                    }
                } break;
            case LLM_ARCH_GEMMA2:
                {
        ...
        case LLM_ARCH_PI0:
            {
                switch (params.gtype) {
                    case LLM_GRAPH_TYPE_ENCODER:
                        llm = std::make_unique<llm_build_pi0>(*this, params);
                        break;
                    case LLM_GRAPH_TYPE_DEFAULT:
                    case LLM_GRAPH_TYPE_DECODER:
                        llm = std::make_unique<llm_build_pi0_ae>(*this, params);
                        break;
                    default:

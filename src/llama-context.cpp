...
static void pi05_binary_dump_tensor_f32(const char * name, ggml_tensor * tensor) {
    if (!pi05_debug_binary_enabled() || tensor == nullptr) {
        return;
    }
    if (tensor->type != GGML_TYPE_F32) {
        LLAMA_LOG_WARN("%s: skipping %s because tensor type is %s, expected f32\n",
...
static std::vector<float> create_normal_noise_cpp11(size_t count, unsigned int seed = 42) {
...
static bool try_load_pi0_action_noise(std::vector<float> & noise) {
    ...
    if (size != expected) {
        LLAMA_LOG_WARN("%s: PI0_ACTION_NOISE_BIN=%s has %lld bytes, expected %lld; falling back to C++ RNG\n",
...
void llama_context::pi0_refresh_encoded_kv_gpu(const std::vector<ggml_tensor *> & src, int n_layer) {
    ...
    if (src_ref == nullptr || src_ref->buffer == nullptr || ggml_backend_buffer_is_host(src_ref->buffer)) {
        cross.pi0_use_gpu_kv = false;
        return;
    }
...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    if (model.arch == LLM_ARCH_PI0) {
        ...
        const auto & t_encoded_kv = res->get_encoded_kv();
        GGML_ASSERT(!t_encoded_kv.empty());
        cross.n_embd  = t_embd != nullptr ? t_embd->ne[0] : hparams.n_embd;
        cross.n_enc   = t_embd != nullptr ? t_embd->ne[1] : n_tokens;
        cross.n_token = t_encoded_kv[0]->ne[2];

        if (pi05_debug_binary_enabled()) {
            pi05_binary_dump_tensor_f32("prefix_k_layer_00", t_encoded_kv[0]);
            if (t_encoded_kv.size() > 1) {
                pi05_binary_dump_tensor_f32("prefix_v_layer_00", t_encoded_kv[1]);
            }
            const size_t last_k = (t_encoded_kv.size() - 1) & ~(size_t) 1;
            pi05_binary_dump_tensor_f32("prefix_k_layer_last", t_encoded_kv[last_k]);
            if (last_k + 1 < t_encoded_kv.size()) {
                pi05_binary_dump_tensor_f32("prefix_v_layer_last", t_encoded_kv[last_k + 1]);
            }
        }
        ...
        try_load_pi0_action_noise(cross.action);
        pi05_debug_dump_f32(
                "action_noise",
                cross.action.data(),
                cross.action.size(),
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    if (model.arch == LLM_ARCH_PI0 && cross.n_token > 0) {
        ...
        const int64_t action_num = hparams.action_steps;
        const int64_t action_dim = hparams.action_dim;
        if (action_num <= 0 || action_dim <= 0) {
            LLAMA_LOG_ERROR("%s: invalid PI0 action shape: steps=%d dim=%d\n", __func__, hparams.action_steps, hparams.action_dim);
            return -1;
        }
        if ((int64_t) cross.action.size() != action_num * action_dim) {
            cross.action = create_normal_noise_cpp11((size_t) action_num * action_dim, 41);
            try_load_pi0_action_noise(cross.action);
        }
        cross.pi0_action_input = cross.action;
        if (cross.state.empty()) {
...

        {
            ...
            const int64_t total_time_elements = action_num * embd_ae;
            const double min_period = 0.004;
            const double max_period = 4.0;
            ...
            const bool cache_valid =
                ...
                cross.ae_time_embeddings.size() == (size_t) hparams.inference_steps * total_time_elements &&
                cross.ae_time_cond.size() == (size_t) hparams.inference_steps * embd_ae;

            if (!cache_valid) {
                cross.ae_time_embeddings.resize((size_t) hparams.inference_steps * total_time_elements);
                ...
                for (uint32_t step = 0; step < hparams.inference_steps; ++step) {
                    ...
                    for (int64_t i = 0; i < half_dim; ++i) {
                        const double fraction = (half_dim > 1) ? (double) i / (half_dim - 1) : 0.0;
                        const double period = min_period * std::pow(max_period / min_period, fraction);
                        scaling_factors[i] = (1.0 / period) * 2.0 * M_PI * time_step;
                    }
                    ...
                    float * time_data = cross.ae_time_embeddings.data() + (size_t) step * total_time_elements;
        ...
        for (int32_t step = 0; step < hparams.inference_steps; ++step) {
            ...
            if (action == nullptr) {
                LLAMA_LOG_ERROR("%s: PI0 decoder graph did not produce action tensor\n", __func__);
                return -3;
            }
            ...
            if (is_pi05 && pi05_debug_binary_enabled()) {
                ...
                pi05_debug_dump_f32(
                        dump_name.c_str(),
                        action_data.data(),
                        (size_t) action_num * (size_t) action_dim,
                        { action_dim, action_num });
            }

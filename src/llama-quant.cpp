...
// Global flag for the new independent interface (MLP linear only quantization).
// Default is false to preserve original behavior.
static bool g_only_mlp_linear = false;
...
static void llama_model_quantize_impl(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    for (const auto * it : tensors) {
        ...
        // ========== Independent new feature: only quantize MLP linear layers ==========
        // When g_only_mlp_linear is true (set only by the new dedicated interface),
        // we skip quantization for all non-MLP-linear weights (keep original precision).
        // This does NOT affect default behavior (g_only_mlp_linear remains false).
        if (g_only_mlp_linear) {
            bool is_mlp_linear =
                ...
                // support for MoE expert variants
...
// =============================================================================
// New independent interface: only quantize MLP linear layers (ffn_up/down/gate)
// All other weights (attention, embeddings, norms, output, etc.) keep original precision.
// This function does NOT change any default behavior of llama_model_quantize.
// Call this when you want MLP-only quantization.
// =============================================================================
uint32_t llama_model_quantize_mlp_linear_only(
        const char * fname_inp,
        const char * fname_out,
        const llama_model_quantize_params * user_params) {
    try {
        // Temporarily enable the MLP-only mode for this call only.
        g_only_mlp_linear = true;

        llama_model_quantize_params params = user_params ? *user_params : llama_model_quantize_default_params();

        // Reasonable defaults for MLP-only scenario
        if (params.ftype == LLAMA_FTYPE_MOSTLY_Q5_1) {
            // keep user's ftype if they set one, otherwise suggest Q4_K_M
            // but we don't override user's choice
        }

        llama_model_quantize_impl(fname_inp, fname_out, &params);

        g_only_mlp_linear = false;  // restore for safety
    } catch (const std::exception & err) {
        g_only_mlp_linear = false;
        LLAMA_LOG_ERROR("%s: failed to quantize MLP linear only: %s\n", __func__, err.what());
        return 1;
    }

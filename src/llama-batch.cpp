...
llama_ubatch llama_batch_allocr::ubatch_add(const std::vector<int32_t> & idxs, uint32_t n_seqs, bool equal_seqs) {
    ...
    for (size_t i = 0; i < single_img_token_num && i < idxs.size(); ++i) {
        ...
        if (batch.embd2) {
            memcpy(udata->embd2.data() + i*n_embd, batch.embd2 + (int64_t) idxs[i]*n_embd, n_embd*sizeof(float));
        }
        if (batch.embd3) {
...
struct llama_batch llama_batch_init_pi0(int32_t n_tokens_alloc, int32_t embd, int32_t n_seq_max) {
    ...
    batch.seq_id   = (llama_seq_id **) malloc(sizeof(llama_seq_id *) * (n_tokens_alloc + 1));
    if (n_tokens_alloc > 0) {
        llama_seq_id * seq_id_storage = (llama_seq_id *) malloc(sizeof(llama_seq_id) * n_tokens_alloc * n_seq_max);
        for (int i = 0; i < n_tokens_alloc; ++i) {
            batch.seq_id[i] = seq_id_storage + (int64_t) i * n_seq_max;
        }
    }
    batch.seq_id[n_tokens_alloc] = nullptr;
    batch.logits = (int8_t *) malloc(sizeof(int8_t) * n_tokens_alloc);

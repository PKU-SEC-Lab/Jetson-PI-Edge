...
bool llm_graph_input_embd_pi0::can_reuse(const llm_graph_params & params) {
    bool res = true;
    const int64_t n_text_tokens = params.ubatch.n_tokens - params.ubatch.img_token_num;
...
void llm_graph_result::reset() {
    ...
    inpL          = nullptr;
    std::fill(encoded_kv.begin(), encoded_kv.end(), nullptr);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    ggml_tensor * cur = build_attn_mha(q_cur, k_cur, v_cur, kq_b, kq_mask, sinks, v_mla, kq_scale, il);
    cb(cur, "kqv_out", il);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    GGML_UNUSED(n_tokens);
    GGML_UNUSED(probs_bf16_roundtrip);

    ...
    def prepare_tensors(self):
        ...
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()):
            ...
            for new_name, data_torch in (self.modify_tensors(data_torch, name, bid)):
                ...
                print(f'name:{name}, new_name:{new_name}')
...
class GemmaModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # NOTE: Gemma RMSNorm uses (1+w) convention. The +1 offset is applied
        # at runtime by build_norm_pi0, so we store the raw weight here.
        # if name.endswith("norm.weight"):
        #     data_torch = data_torch + 1

        return [(self.map_tensor_name(name), data_torch)]

@ModelBase.register("pi0")
class PI0Model(TextModel):
    ...
    # "intermediate_size_ae": 4096,
        self.gguf_writer.add_embedding_length_ae(hparams["hidden_size_ae"])
        self.gguf_writer.add_feed_forward_length_ae(hparams["intermediate_size_ae"])
        self.gguf_writer.add_inference_steps(inference_steps)
        self.gguf_writer.add_n_action_steps(action_steps)
        self.gguf_writer.add_n_action_dim(action_dim)
        self.gguf_writer.add_bool(
            f"{gguf.MODEL_ARCH_NAMES[self.model_arch]}.pi05",
            bool(hparams.get("pi05", "time_mlp_in.weight" in self.model_tensors)),
        )
        # self.gguf_writer.add_vocab_size(hparams["vocab_size"])
        self.gguf_writer.add_layer_norm_eps(1e-6)

    def _is_pi05_tensor(self, name: str) -> bool:
    ...
    def _get_attention_heads_for_tensor(self, name: str) -> tuple[int, int] | None:
        # Gemma/PaliGemma Q/K weights do not use LLaMA-style interleaved layout,
        # so we skip the permute entirely by always returning None.
        return None

    

    def tensor_force_quant(self, name, new_name, bid, n_dims):
        """Force PI0-specific tensors to F32 to match PyTorch mixed-precision.

        In PyTorch inference, core Gemma/expert weights are BF16 but
        PI0-specific modules (action proj, time MLP, norm dense) are F32.
        """
        f32_patterns = [
            "action_in_proj", "action_out_proj",
            "time_mlp_in", "time_mlp_out",
            "attn_norm_dense", "ffn_norm_dense", "output_norm_dense",
        ]
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if is_multimodal_tensor:
            ...
            name = name.replace("paligemma_with_expert.paligemma.", "")
            if "lm_head.weight" in name:
                # paligemma_with_expert.paligemma.lm_head.weight
                if any(
                    tensor_name.endswith("language_model.embed_tokens.weight")
                    for tensor_name in self.model_tensors
                ):
                    logger.info("Skipping unused PaliGemma output head because input embeddings are present")
                    return []
                name = name.replace("lm_head.", "token_embd.")
        elif name.startswith("paligemma_with_expert.gemma_expert."):
            name = name.replace("paligemma_with_expert.gemma_expert.", "ae.")
            layer_num = re.findall(r'\d+', name)
            if len(layer_num) != 0:
                name = name.replace(layer_num[0], f"{int(layer_num[0]) + self.block_count // 2}")
            if "lm_head.weight" in name:
                logger.debug(f"Skipping get tensor {name!r} in safetensors so that convert can end normally.")
                return []
        if name in ('model.language_model.norm.weight', 'language_model.norm.weight'):
            return []
        if "language_model." in name:
            name = name.replace("language_model.", "") # for the rest

        attention_heads = self._get_attention_heads_for_tensor(name)
        if attention_heads is not None:
            n_head, n_kv_head = attention_heads
            if name.endswith(("q_proj.weight", "q_proj.bias")):
                data_torch = LlamaModel.permute(data_torch, n_head, n_head)
            if name.endswith(("k_proj.weight", "k_proj.bias")):
                data_torch = LlamaModel.permute(data_torch, n_head, n_kv_head)

        is_pi05_tensor = self._is_pi05_tensor(name)
        ...
        if name == "ae.model.norm.weight":
            if any(
                tensor_name == "ae.model.norm.dense.weight"
                or tensor_name.endswith("gemma_expert.model.norm.dense.weight")
                for tensor_name in self.model_tensors
            ):
                logger.info("Skipping unused Pi0.5 base output norm because adaptive output norm is present")
                return []
            self.tensor_not_map_num += 1
            print('tensor_not_map_num', self.tensor_not_map_num)
            return [("output_norm.weight", data_torch)]

        if name.startswith("ae.model.norm.dense"):
            suffix = ".weight" if name.endswith(".weight") else ".bias" if name.endswith(".bias") else ""
            self.tensor_not_map_num += 1
            print('tensor_not_map_num', self.tensor_not_map_num)
            return [(f"output_norm_dense{suffix}", data_torch)]

        if name.startswith("ae.model.layers."):
            name = name.replace("ae.model.layers.", "model.layers.", 1)

        pi05_layer_match = re.match(
            r"model\.layers\.(\d+)\.(input_layernorm|post_attention_layernorm)\.dense\.(weight|bias)$",
            name,
        )
        if pi05_layer_match is not None:
            layer, norm_name, suffix = pi05_layer_match.groups()
            mapped_norm = "attn_norm_dense" if norm_name == "input_layernorm" else "ffn_norm_dense"
            self.tensor_not_map_num += 1
            print('tensor_not_map_num', self.tensor_not_map_num)
            return [(f"blk.{layer}.{mapped_norm}.{suffix}", data_torch)]

        if 'action' in name or 'state' in name or 'ae' in name or is_pi05_tensor:
            self.tensor_not_map_num += 1
            print('tensor_not_map_num',self.tensor_not_map_num)
            return [(self.map_tensor_name(name), data_torch)]
        else:
            self.tensor_num += 1
            print('tensor_num',self.tensor_num)
            return [(self.map_tensor_name(name), data_torch)]
...
# The Gemma3VisionModel class sets GGUF parameters for a vision model, including projector type,
# attention layernorm epsilon, GELU usage, and projector scale factor calculation.

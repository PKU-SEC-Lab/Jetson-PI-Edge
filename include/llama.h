...
extern "C" {
    ...
    typedef struct llama_batch {
        ...
        float        *  embd2;
        float        *  embd3;
        int32_t      img_token_num;          // TODO: remove --- IGNORE ---
        int32_t      single_img_token_num;   // TODO: remove --- IGNORE
        llama_pos    *  pos;
        int32_t      *  n_seq_id;
        llama_seq_id ** seq_id;
...
#define LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY 1
    ...
    LLAMA_API struct llama_batch llama_batch_init_pi0(
            int32_t n_tokens,
            int32_t embd,
            int32_t n_seq_max);
    ...
    LLAMA_API void llama_batch_free_pi0(struct llama_batch batch);
    ...
    // Get the current pi0 state tensor from the last decoder iteration.
    // Returns NULL when the current model is not pi0 or no state result is available.
    // shape: [state_dim]
    LLAMA_API const float * llama_get_pi0_state(struct llama_context * ctx);
    LLAMA_API int32_t llama_get_pi0_state_size(struct llama_context * ctx);

    // Get the pi0 action tensor before the decoder refinement loop.
    // Returns NULL when the current model is not pi0 or no action input is available.
    // shape: [action_steps * action_dim] (flattened row-major by step)
    LLAMA_API const float * llama_get_pi0_action_input(struct llama_context * ctx);

    // Get the current pi0 action tensor after llama_decode().
    // Returns NULL when the current model is not pi0 or no action result is available.
    // shape: [action_steps * action_dim] (flattened row-major by step)
    LLAMA_API const float * llama_get_pi0_action(struct llama_context * ctx);
    LLAMA_API int32_t llama_get_pi0_action_dim(struct llama_context * ctx);
    LLAMA_API int32_t llama_get_pi0_action_steps(struct llama_context * ctx);

    // Set the pi0 state vector on a pi0 context.
    // `state_array` is interpreted as float data, not serialized session state.
